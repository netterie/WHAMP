...
Prior to launching the Washington HIV Prevention Project (hereafter referred to as the internet survey), the majority of data on HIV-related behaviors focused on King County. The Department of Health was interested in gathering data from men in other regions to understand how their prevention needs and behaviors may be different. For the internet survey, we defined three regions: King County, other counties in western Washington, and Eastern Washington^[Western Washington outside of King County was defined to include the following counties: Clallam, Clark, Cowlitz, Grays Harbor, Island, Jefferson, Kitsap, Lewis, Mason, Pacific, Pierce, San Juan, Skagit, Skamania, Snohomish, Thurston, and Whatcom. Eastern Washington includes Adams, Asotin, Benton, Chelan, Douglas, Ferry, Franklin, Grant, Kittitas, Klickitat, Okanogan, Pend Oreille, Spokane, Stevens, Walla Walla, Whitman, and Yakima Counties.] In the course of descriptive analyses of the internet survey data, we observed that there were notable differences by region on factors such as PrEP utilization and HIV testing. As such, we felt it would be important to represent regional heterogeneity in the model. <span style="color:green">To identify parameters that vary by region independent of age and race effects, we conducted regression analyses, using the Bonferroni correction to adjust for multiple comparisons. These analyses indicated significant regional variability in</span><span style="color:green">XXXX, XXXX, and XXXX</span><span style="color:green"> (see section @ref(ADDLABEL)), so these parameters will be stratified by region.</span>.

### Regional distribution
To define the distribution of nodes in the network by region, we need to know the distribution of MSM by region in the population. The proportion of respondents to the internet survey could provide some indication of this distribution, but there are likely factors that may have differntially influenced whether men saw and clicked the ads and completed the survey by region. However, the distribution of males in Washington recorded by the census also likely doesn't reflect the distribution of MSM; I would expect more MSM to preferentially migrate to King County than heterosexual men. An alternative source of information on the regional distribution of MSM is a 6201 analysis by Jeremy Grey et al. [@Grey2016] that estimated the sizes of the MSM population by state and county. County-level data are not published, but <span style="color:green">I may be able to obtain these data for Washington</span>. Table @ref(tab:region) shows the differences between the regional composition of the internet sample, and the regional distribution of males in Washington based on the 2011-2015 American Community Survey 5-year Estimates [@WA-census]^[Data were downloaded from the American FactFinder using the Guided Search. I selected the option "I'm looking for information about people", then under topics I selected Age and Sex from the "Age & Sex" menu. For Geographies, I selected County as the geographic type, Washington as the state, clicked "All counties within Washington," and added my selection. For Race/Ethnic Groups, I clicked "Select from detailed groups", and added the following to my selection: Total population (code 001), Hispanic or Latino (of any race)(200-299) (code 400), White alone, not Hispanic or Latino (code 451), Black or African American alone, not Hispanic or Latino (code 453), Black or African American alone or in combination with one or more other races, not Hispanic or Latino (code 454), American Indian and Alaska Native alone, not Hispanic or Latino (code 455), Asian alone, not Hispanic or Latino (code 457), Native Hawaiian and Other Pacific Islander alone, not Hispanic or Latino (code 459), Some other race alone, not Hispanic or Latino (code 461), Two or more races, not Hispanic or Latino (code 463), Black or African American alone, Hispanic or Latino (code 466), and Black or African American alone or in combination with one or more other races, Hispanic or Latino (code 467). I selected the latter two to look at how many men identified as Hispanic and reported black race. From the available tables, I clicked "SEX BY AGE", table ID B01001, and downloaded it with the option to use the data, with both options selected.], <span style="color:green">and the distribution implied by the estimates from @Grey2016.</span> 
...
```{r region}
    ...
    #Combine columns
    region <- cbind.data.frame("Age group" = sample_region$region,  "Census" = (census_region$percent), "Sample" = (sample_region$percent), "Grey et al." = c("TBD", "TBD", "TBD"))
    
    #make table
    kable(region, col.names=c("Region", "Census", "Sample", "Grey et al."), digits=c(0, 3, 3, 3), caption="Regional distribution: Census, interent survey sample, and Grey et al. 2016") %>% kable_styling(full_width=F, position="center")
```
