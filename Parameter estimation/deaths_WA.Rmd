...
# DEFINE AGE GROUPS #
wa_mort_males$age_cat = cut(wa_mort_males$age, c(17, 19, 24, 29, 34, 39, 44, 49, 54, 59), labels=c("18-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59"))
...
nums <- cbind.data.frame("age_cat" = c("18-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59"), "Black" = c(wa_mort_count$`n()`[wa_mort_count$hbo %in% "Black"]), "Hispanic" = c(wa_mort_count$`n()`[wa_mort_count$hbo %in% "Hispanic"]), "Other" = c(wa_mort_count$`n()`[wa_mort_count$hbo %in% "Other"]))
...
#Re-group ages 20-24 into bin 20-24
    wa_acs_male18to59$males.20to24 <- (wa_acs_male18to59$males.20 +  wa_acs_male18to59$males.21 + wa_acs_male18to59$males.22to24)

#Select only age groups of interest
    wa_acs_male18to59 <- wa_acs_male18to59 %>% select(county, Population.Group, males.18to19, males.20to24, males.25to29, males.30to34, males.35to39, males.40to44, males.45to49, males.50to54, males.55to59)
...
#Define race groups - note: this is not perfect because the numbers for the individuals race/eth groups (Hispanic, NH white, NH black alone, NH Asian, NH Native Hawaiian/Other Pacific Islander, NH American Indian/Alaska Native, NH other, and NH two or more) do not sum to the total in each county. This is likely due to variability in the estimates for each group. Could keep the columns with the margin of error for the original data frame and work with those (details on how they are calculated here: https://www.census.gov/content/dam/Census/programs-surveys/acs/guidance/training-presentations/2016_MOE_Transcript_01.pdf. But for now use use the point estimates and define other group as total - Hispanic - black alone or in combination
    ...
    wa_acs_male18to59_hbo = within(wa_acs_male18to59_hbo, {
                            age_cat = ifelse(Age_group %in% "males.18to19", "18-19",
                                             ifelse(Age_group %in% "males.20to24", "20-24",
