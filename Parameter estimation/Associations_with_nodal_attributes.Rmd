...
### Circumcision
<span style = "color:red"> Add analysis of circumcision by age and race from NHBS data </span>
...
For epidemic model parameters, we will explore **regional differences in the proportion never tested**, **age differences in the intertest interval**, **regional differences in coital frequency in main partnerships**, **age and racial/ethnic differences in anal sex role**, **and age differences in condom use in main partnerships**. For **PrEP use, we will exlpore differences by all three nodal attributes**.
...
__Decision__: There seems to be an age effect on the rate of instantaneous partnerships, and in the quantile disutribution plots, the rate appears to differ by main and persistent degree. This latter effect is important because it implies different levels of concurrency. There is not strong evidence of an interaction between age and momentary degree. As such, we will represent main effects on the rate of instantaneous partnerships for both momentary degree and age. For momentary degree, we will stratify the rate of instantaneous partnerships by the joint distribution of main and persistent partnership status. For age, although the rate of instantaneous partnerships decreases slightly from ages 18 to 30, the biggest change appears to be an increase in the proportion with zero instantaneous partners at age 50. So we will assign everyone to an instantaneous partnership quantile at entry to the population, and at age 50 we will adjust the rates within these quantiles to account for the fact that men aged 50+ have lower rates of instantaneous partnerships. 
...
### Coital frequency by region
The regression analyses indicated that coital frequency varies by region in main partnerships. In the plots below, we show the mean coital frequency in both main and perssitent partnerships by region. The error bars indicate the 95% confidence interval of the point estimate.

```{r plotsexplore_coitalfreq}
#Coital frequency in main partnerships by region
  airate_mainXreg <- sample %>% filter(!is.na(airate_main)) %>% group_by(region) %>% summarise(freq=n(), mean = mean(airate_main), se=sd(airate_main)/sqrt(freq), ci_lower = max((mean - 1.96*se), 0), ci_upper = min((mean + 1.96*se), 1))

  ggplot(airate_mainXreg) + geom_pointrange(aes(x=region, y=mean, ymin=ci_lower, ymax=ci_upper)) + labs(x="Region", y="Mean coital frequency", title = "Mean coital frequency in main partnerships by region") + theme_title + plot_background
  
    airate_persXreg <- sample %>% filter(!is.na(airate_pers)) %>% group_by(region) %>% summarise(freq=n(), mean = mean(airate_pers), se=sd(airate_pers)/sqrt(freq), ci_lower = max((mean - 1.96*se), 0), ci_upper = min((mean + 1.96*se), 1))

  ggplot(airate_persXreg) + geom_pointrange(aes(x=region, y=mean, ymin=ci_lower, ymax=ci_upper)) + labs(x="Region", y="Mean coital frequency", title = "Mean coital frequency in persistent partnerships by region") + theme_title + plot_background
    ```

__Decision__: </span style="color:red"> These plots indicate that coital freuqency varies by region in main partnerships. </span>
