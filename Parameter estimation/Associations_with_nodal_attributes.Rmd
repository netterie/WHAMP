...
The parameter indicating the proportion of men who never test is meant to indicate the proportion of men who will not be screened for HIV. As documented in section \@ref(hivtesting), it appears that if men do not test by age 40 they are unlikely to. As such, we will estimate the proportion of men who never test for HIV based on the proportion of men aged 40 and above who report never having tested. <span style="color:red">This analysis includes men who have taken PrEP, because many men who go on PrEP are likely to have come in for testing as the precipitating event to initiating PrEP. Excluding them would overestimate the proportion never tested. </span> To see if this varies by race and region, we will model the probability of having never tested among men aged 40 to 59.
...
```{r iti, eval=TRUE}
 #Define age categorization
  sample$age_cat_iti <- cut(sample_p12noprep$age, c(17, 24, 34, 59), labels=c("18-24", "25-34", "35-59")) 
...
These plots do not suggest any clear differences in the rate of instantaneous partnerships by age. To further examine the distribution of the rate of instantaneous partnerships by momentary degree, the plots below examine whether the percent of men in each quantile by age varies by main and persistent partnership status. <span style="color:red">Red</span> indicates zero instantaneous partnerships, <span style="color:orange">orange</span> indicates a low rate, <span style="color:blue">blue</span> indicates a medium rate, and <span style="color:green">green</span> indicates a high rate.
...
__Decision__: There seems to be an age effect on the rate of instantaneous partnerships, but it does not appear to vary meaningfully by main or persisent degree. As such, we will define the rate of instantaneous partnerships to vary by age but not by degree. <span style="color:red"> To capture the age effect, we could include terms for centered age (age - mean age) and centered age squared, or we could define a transition matrix wherein men move through the quartiles of the distribution as they age. Plot \@ref(fig:rate_inst_fit) shows the rate of instantanoues partnerships by age with the lowess smooth (in blue) and the predicted rate from a model adjusting for centered age and centered age squared (red). __Steve, which approach seems better to you?__</span>

```{r rate_inst_fit}
#Define centered age and centered age squared variables
sample$age_centered <- sample$age - mean(sample$age, na.rm=TRUE)
sample$age_cent_sq <- sample$age_centered^2

#Fit linear model
lm_rate_instXage <- lm(rate_inst ~ age_centered + age_cent_sq, data=sample)

#Create a data frame with prediction from the model and age
rate_inst_pred <- cbind.data.frame(prediction = predict(lm_rate_instXage, list(age_centered = sample$age_centered, age_cent_sq = sample$age_centered^2)), age = sample$age)

#Plot
ggplot(sample[!is.na(sample$rate_inst) & sample$rate_inst <0.25,], aes(x=age, y=rate_inst)) + geom_point() + geom_smooth(method="loess") + geom_line(data=rate_inst_pred, aes(x=age, y=prediction), colour="red") + labs(x="Ego age", y="Rate of instantaneous partnerships", title="Rate of instantaneous partnerships by ego age, \n outliers removed and with fitted regression line") + plot_background + theme_title 
...
Plot \@ref(fig:iti_fit) below shows the median intertest interval by age with a superimposed line with predicted values from a linear model for median intertest interval that includes centered age and centered age squared terms. <span style="color:red"> Does this seem like an okay fit? </span>
...
__Decision__: These plots suggest that there is a higher probability of exclusive bottoming for younger men and a lower probability of topping, but that these probabilities converge by age 30 and remain roughly similar. <span style="color:red"> __Steve, can you think of a good way to capture this? It's complicated because this has implications for age mixing since position is in the network model as an offset. Martina and I felt that, if there's no easy way to represent this, we can let it go.__</span>
...
#Condom use in main partnerships by ego age
   p_condoms_main <- ggplot(sample_p12noprep[!is.na(sample_p12noprep$condoms_main), ], aes(x=age, y=condoms_main)) + geom_point() + labs(x="Ego age", y="Probability of condom use", title = "Probability of condom use in main partnerships by respondent age") + plot_background + theme_title
   
    condoms_mainXage <- sample_p12noprep %>% filter(!is.na(condoms_main)) %>% group_by(age_cat) %>% summarise(freq=n(), mean=mean(condoms_main))
...
#Condom use in persistent partnerships by ego age
 p_condoms_pers <- ggplot(sample_p12noprep[!is.na(sample_p12noprep$condoms_pers),], aes(x=age, y=condoms_pers)) + geom_point() + labs(x="Ego age", y="Probability of condom use", title = "Probability of condom use in persistent partnerships by respondent age") + plot_background + theme_title
   
    condoms_persXage <- sample_p12noprep %>% filter(!is.na(condoms_pers)) %>% group_by(age_cat) %>% summarise(freq=n(), mean=mean(condoms_pers))
...
#Condom use in instantaneous partnerships by ego age
   condoms_instXage <- sample_p12noprep %>% filter(!is.na(condoms_inst)) %>% group_by(age_cat) %>% summarise(freq=n(), mean=mean(condoms_inst))
...
The plots below superimpose predicted condom use from a regression model adjusting for centered age and centered age squared (red) and from a regression model additionally adjusting for centered age cubed (green). It looks like the model with the age cubed term fits the data for main partnerships slightly better, but it perhaps puts too much weight on the high reported condom use at the end of the age distribution. Similarly, the regression line from the model with the age cubed term may fit the persistent partner data slightly better except at the ends of the distribution. The regression models for instantaneous partnerships definitely do not fit the data well: the predictions for ages 18 to 25 are negative, and the data plotted with single years of age are much more scattered due to small cell counts. <span style="color:red"> What do you make of this? How should I proceed? Note that these plots exclude men who have taken PrEP in the past 12 months, which we may decide not to do. </span>

```{r condoms_main_fit}
##MAIN PARTNERS##
#Define a new tibble that takes the mean condom use by age rather than age group
condoms_mainXageinyrs <- sample_p12noprep %>% filter(!is.na(condoms_main)) %>% group_by(age) %>% summarise(freq=n(), mean=mean(condoms_main))

#Define a variable for centered age cubed
sample_p12noprep$age_cent_cube <- sample_p12noprep$age_centered^3

#Fit linear model to the mean condom use, adjusting for centered age and centered age squared
lm_condoms_mainXage2 <- lm(condoms_main ~ age_centered + age_cent_sq, data=sample_p12noprep)

#Fit linear model to the mean condom use, adjusting for centered age, centered age squared, and centered age cubed
lm_condoms_mainXage3 <- lm(condoms_main ~ age_centered + age_cent_sq + age_cent_cube, data=sample_p12noprep)

#Create a data frame with predictions from the models and age
condoms_main_pred_age2 <- cbind.data.frame(prediction = predict(lm_condoms_mainXage2, list(age_centered = sample_p12noprep$age_centered, age_cent_sq = (sample_p12noprep$age_centered)^2)), age = sample_p12noprep$age)

condoms_main_pred_age3 <- cbind.data.frame(prediction = predict(lm_condoms_mainXage3, list(age_centered = sample_p12noprep$age_centered, age_cent_sq = (sample_p12noprep$age_centered)^2, age_cent_cube = (sample_p12noprep$age_centered)^3)), age = sample_p12noprep$age)

#Plot
 ggplot() + geom_point(data=condoms_mainXageinyrs, aes(x=age, y=mean, size=freq)) + geom_line(data=condoms_main_pred_age2, aes(x=age, y=prediction), colour="red") + geom_line(data=condoms_main_pred_age3, aes(x=age, y=prediction), colour="green") + labs(x="Ego age", y="Mean probability of condom use", title = "Mean probability of condom use in main partnerships by respondent age \n with predicted condom use from a regression model ") + ylim(0, 1) + theme_title + plot_background
```

```{r condoms_pers_fit}
##PERSISTENT PARTNERS##
condoms_persXageinyrs <- sample_p12noprep %>% filter(!is.na(condoms_pers)) %>% group_by(age) %>% summarise(freq=n(), mean=mean(condoms_pers))

#Fit linear model to the mean condom use, adjusting for centered age and centered age squared
lm_condoms_persXage2 <- lm(condoms_pers ~ age_centered + age_cent_sq, data=sample_p12noprep)

#Fit linear model to the mean condom use, adjusting for centered age, centered age squared, and centered age cubed
lm_condoms_persXage3 <- lm(condoms_pers ~ age_centered + age_cent_sq + age_cent_cube, data=sample_p12noprep)

#Create a data frame with predictions from the models and age
condoms_pers_pred_age2 <- cbind.data.frame(prediction = predict(lm_condoms_persXage2, list(age_centered = sample_p12noprep$age_centered, age_cent_sq = (sample_p12noprep$age_centered)^2)), age = sample_p12noprep$age)

condoms_pers_pred_age3 <- cbind.data.frame(prediction = predict(lm_condoms_persXage3, list(age_centered = sample_p12noprep$age_centered, age_cent_sq = (sample_p12noprep$age_centered)^2, age_cent_cube = (sample_p12noprep$age_centered)^3)), age = sample_p12noprep$age)

#Plot
 ggplot() + geom_point(data=condoms_persXageinyrs, aes(x=age, y=mean, size=freq)) + geom_line(data=condoms_pers_pred_age2, aes(x=age, y=prediction), colour="red") + geom_line(data=condoms_pers_pred_age3, aes(x=age, y=prediction), colour="green") + labs(x="Ego age", y="Mean probability of condom use", title = "Mean probability of condom use in persistent partnerships by respondent age \n with predicted condom use from a regression model ") + ylim(0, 1) + theme_title + plot_background
```

```{r condoms_inst_fit}
##INST PARTNERS##
condoms_instXageinyrs <- sample_p12noprep %>% filter(!is.na(condoms_inst)) %>% group_by(age) %>% summarise(freq=n(), mean=mean(condoms_inst))

#Fit linear model to the mean condom use, adjusting for centered age and centered age squared
lm_condoms_instXage2 <- glm(condoms_inst ~ age_centered + age_cent_sq, family=binomial(link="logit"), data=sample_p12noprep, subset = prep_use_r %in% "Never taken PrEP")

#Fit linear model to the mean condom use, adjusting for centered age, centered age squared, and centered age cubed
lm_condoms_instXage3 <- glm(condoms_inst ~ age_centered + age_cent_sq + age_cent_cube, family=binomial(link="logit"), data=sample_p12noprep, subset = prep_use_r %in% "Never taken PrEP")

#Create a data frame with predictions from the models and age
condoms_inst_pred_age2 <- cbind.data.frame(prediction = predict(lm_condoms_instXage2, list(age_centered = sample_p12noprep$age_centered, age_cent_sq = (sample_p12noprep$age_centered)^2)), age = sample_p12noprep$age)

condoms_inst_pred_age3 <- cbind.data.frame(prediction = predict(lm_condoms_instXage3, list(age_centered = sample_p12noprep$age_centered, age_cent_sq = (sample_p12noprep$age_centered)^2, age_cent_cube = (sample_p12noprep$age_centered)^3)), age = sample_p12noprep$age)

#Plot
 ggplot() + geom_point(data=condoms_instXageinyrs, aes(x=age, y=mean, size=freq)) + geom_line(data=condoms_inst_pred_age2, aes(x=age, y=prediction), colour="red") + geom_line(data=condoms_inst_pred_age3, aes(x=age, y=prediction), colour="green") + labs(x="Ego age", y="Mean probability of condom use", title = "Mean probability of condom use in instantaneous partnerships by respondent age \n with predicted condom use from a regression model ") + ylim(0, 1) + theme_title + plot_background
```
