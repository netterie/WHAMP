...
#Age as binary 40-49 vs all else
 ...
 #Interactions
  ...
  results_anypers_age40to49 <- compare.models(anypers_full_age40to49, anypers_noregion_age40to49, anypers_norace_age40to49, anypers_noage_age40to49,  anypers_full_withint_age40to49, anypers_noraceXreg_age40to49, anypers_noageXrace_age40to49, anypers_noageXreg_40to49)
...
#Age as binary 40-49 vs all else
 ...
 #Interactions
  ...
  results_concurrent_mainpers_age40to49 <- compare.models(concurrent_mainpers_full_age40to49, concurrent_mainpers_noregion_age40to49, concurrent_mainpers_norace_age40to49, concurrent_mainpers_noage_age40to49,  concurrent_mainpers_full_withint_age40to49, concurrent_mainpers_noraceXreg_age40to49, concurrent_mainpers_noageXrace_age40to49, concurrent_mainpers_noageXreg_40to49)
...
```{r}
 #Figure out how to parameterize age
    ...
    p2 <- ggplot(agecoefs_dropcat, aes(x=age_cat, y=coefs)) + geom_pointrange(aes(ymin=coefs - 1.96*se, ymax=coefs + 1.96*se)) + labs(x="Age groups", y="Adjusted coefficients and CI", title="Adjusted coefficients and 95% confidence intervals for age, dropping the 55-59 year-old group") + theme_title + plot_background
...
With the exception of the estimate for ages 50-54, there does not appear to be a meaningful trend by age. <span style="color:red">However, to be consistent with the approach taken for main partners, we will model age as a categorical variable, collapsing ages 40-59 into 10-year bins. Although this groups together ages 50-54 and 55-59, for which the estimates appear different, there are only 6 and 4 men in each age group. </span>
...
```{r}
  #Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$iti) & !(sample$prep_use_r %in% "Currently taking PrEP")])
  table(sample$age_cat[!is.na(sample$race_eth_m) & !is.na(sample$iti) & !(sample$prep_use_r %in% "Currently taking PrEP")])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$iti) & !(sample$prep_use_r %in% "Currently taking PrEP")])
...
```{r iti, eval=TRUE}
 ...
 #Main effects
  iti_full <- lm(iti ~ age_cat_iti + race_eth_m.reg + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noregion <- lm(iti ~ age_cat_iti + race_eth_m.reg, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_norace <- lm(iti ~ age_cat_iti + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noage <- lm(iti ~ race_eth_m.reg + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))

 #Interactions
  iti_full_withint <- lm(iti ~ age_cat_iti + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + age_cat_iti*region.bKC + race_eth_m.reg*age_cat_iti, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noraceXreg <- lm(iti ~ age_cat_iti + race_eth_m.reg + region.bKC + age_cat_iti*region.bKC + race_eth_m.reg*age_cat_iti, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noageXreg <- lm(iti ~ age_cat_iti + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + race_eth_m.reg*age_cat_iti, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noraceXage <- lm(iti ~ age_cat_iti + race_eth_m.reg + region.bKC + age_cat_iti*region.bKC + race_eth_m.reg*region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
...
```{r}
  ...
  #Plot univariate associations
    #    positionXage <- sample %>% filter(!is.na(position_cat)) %>% group_by(age_cat, position_cat) %>% summarise(freq=n()) %>% group_by(age_cat) %>% mutate(percent = freq/sum(freq))
    #   positionXrace <- sample %>% filter(!is.na(position_cat)) %>% group_by(hbo, position_cat) %>% summarise(freq=n()) %>% group_by(hbo) %>% mutate(percent = freq/sum(freq))
    #   positionXregion <- sample %>% filter(!is.na(position_cat)) %>% group_by(region, position_cat) %>% summarise(freq=n()) %>% group_by(region) %>% mutate(percent = freq/sum(freq))
    # 
    # ggplot(positionXage, aes(x=age_cat, y=percent, color=position_cat)) + geom_point()
    # ggplot(positionXrace, aes(x=hbo, y=percent, color=position_cat)) + geom_point()
    # ggplot(positionXregion, aes(x=region, y=percent, color=position_cat)) + geom_point()
...
```{r sexrole}
 ...
 #Interactions
  ...
  top_noregionXage <- glm(position_top ~ age_cat_alt + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + race_eth_m.reg*age_cat_alt, family=binomial(link="logit"), data=sample)
  
  results_topping <- compare.models(top_full, top_noregion, top_norace, top_noage, top_full_withint, top_noraceXreg, top_noraceXage, top_noregionXage)
  results_topping
...
```{r sexrole}
 ...
 #Interactions
  ...
  bottom_noregionXage <- glm(position_bottom ~ age_cat_alt + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + race_eth_m.reg*age_cat_alt, family=binomial(link="logit"), data=sample)
  
  results_bottoming <- compare.models(bottom_full, bottom_noregion, bottom_norace, bottom_noage, bottom_full_withint, bottom_noraceXreg, bottom_noraceXage, bottom_noregionXage)
  results_bottoming
...
```{r sexrole}
 ...
 #Interactions
  ...
  versatile_noregionXage <- glm(position_versatile ~ age_cat_alt + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + race_eth_m.reg*age_cat_alt, family=binomial(link="logit"), data=sample)
  
  results_versatile <- compare.models(versatile_full, versatile_noregion, versatile_norace, versatile_noage, versatile_full_withint, versatile_noraceXreg, versatile_noraceXage, versatile_noregionXage)
  results_versatile
...
```{r}
 #Figure out how to parameterize age
    ...
    p2 <- ggplot(agecoefs_condoms_inst, aes(x=age_cat, y=coefs)) + geom_pointrange(aes(ymin=coefs - 1.96*se, ymax=coefs + 1.96*se)) + labs(x="Age groups", y="Adjusted coefficients and CI", title="Adjusted coefficients and 95% confidence intervals for age, dropping the 55-59 year-old group") + theme_title + plot_background + ylim(-3, 5)
...
## Summary tables
<span style="color:red"> UPDATE - will ned to change the indexing for results as the result objects no longer contain the summary of the models as the first and third list items (now only the p-values from the LR tests) and age is not tested for some covariates - put as NA for those.</span>
