...
The parameter indicating the proportion of men who never test is meant to indicate the proportion of men who will not be screened for HIV. In previous network models, this was defined as the proportion who never tested by age 40, since the model was capped at age 39. For this model, that may not be appropriate. For the current analysis, I simply modeled the probability that men had ever tested at their current age.</span>
...
### HIV testing by age
The first graph below looks at the proportoin of men who report having ever tested by age group. The graph is a bit tricky to interpret because the figure represents both age and cohort effects, and because the number of respondents for some ages is small. It seems that if men don't test by age 40-45, they are unlikely to get tested. The pattern is similar if age is binned in 5 year groups.
...
#Intertest interval by age (with calculated mean as implied by exponential distribution)
    itiXage <- sample %>% filter(!is.na(iti) & prep_use_r !="Currently taking PrEP") %>% group_by(age) %>% summarise(freq=n(), median=median(iti, na.rm=TRUE), int_mean = 2*mean(iti, na.rm=TRUE), int_median=2*median(iti, na.rm=TRUE)) %>% mutate(exp_mean = median/log(2))
    
    ggplot(itiXage) + geom_point(aes(x=age, y=int_median, size=freq), colour="blue") + geom_point(aes(x=age, y=int_mean, size=freq), colour="red") + geom_smooth(aes(x=age, y=int_median), method="loess") + geom_smooth(aes(x=age, y=int_mean), method="loess") + labs(x="Ego age", y="Median days since last test", title="Median intertest interval by age, assuming testing is an inteval process")
