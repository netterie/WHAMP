...
plot.age.coefs <- function(full) {
    ...
    plot <- ggplot(agecoefs, aes(x=age_cat, y=coefs)) + geom_pointrange(aes(ymin=coefs - 2*se, ymax=coefs + 2*se))
    return(plot)
}
...
The first plot (\@ref(fig:plots_explore_somepartners)) shows the proportion of men reporting at least one ongoing partner by age. Plot \@ref(fig:plots_explore_concurrent) shows the proportion of men reporting two or more concurrent partners by age.

```{r plots_explore_somepartners}
