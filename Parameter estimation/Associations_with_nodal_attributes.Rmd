...
This plot suggests that the rate of instantaneous partners is higher for men aged 35-39 and subsequently decreases with age. The point for ages 30-34 does not fit with a trend suggesting that the rate increases with age up through age 39, but it may be an outlier. To further explore the association with age, we will include age as a dummy variable, grouping together men in age groups 40-49 and 50-59.
...
These coefficients suggest that racial/ethnic homophily is lower among those aged 30-34 and 50-54 and higher among those aged 40-44 and 55-59, though these latter two estimates are not statistically significant. This does not suggest any meaningful trend, so we will not include age in the models below to test for differences by region.
...
```{r}
#Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$sqrt_agediff) & mrp_type_r %in% "One time"])
  table(sample$age_cat[!is.na(sample$race_eth_m) & !is.na(sample$sqrt_agediff) & mrp_type_r %in% "One time"])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$sqrt_agediff) & mrp_type_r %in% "One time"])
...
The parameter indicating the proportion of men who never test is meant to indicate the proportion of men who will not be screened for HIV. As documented in section \@ref(hivtesting), it appears that if men do not test by age 40 they are unlikely to. As such, we will estimate the proportion of men who never test for HIV based on the proportion of men aged 40 and above who report never having tested. To see if this varies by race and region, we will model the probability of having never tested among men aged 40 to 59.

We first look at the number of observations that are non-missing for this outcome and for model predictors.
```{r}
   #Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$evertest_r) & !(sample$prep_use_r %in% "Currently taking PrEP") & sample$age %in% c(40:59)])
  table(sample$age[!is.na(sample$race_eth_m) & !is.na(sample$evertest_r) & !(sample$prep_use_r %in% "Currently taking PrEP") & sample$age %in% c(40:59)])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$evertest_r) & !(sample$prep_use_r %in% "Currently taking PrEP") & sample$age %in% c(40:59)])
...
```{r nevertest}
 ...
 #Main effects
  nevertest_full <- glm(nevertest ~ race_eth_m.reg + region.bKC, family=binomial(link="logit"), data=sample, subset = (!(prep_use_r %in% "Currently taking PrEP") & age %in% c(40:59)))
  nevertest_noregion <- glm(nevertest ~ race_eth_m.reg, family=binomial(link="logit"), data=sample, subset = (!(prep_use_r %in% "Currently taking PrEP") & age %in% c(40:59)))
  nevertest_norace <- glm(nevertest ~ age_cat + region.bKC, family=binomial(link="logit"), data=sample, subset = (!(prep_use_r %in% "Currently taking PrEP") & age %in% c(40:59)))

 #Interactions
  nevertest_full_withint <- glm(nevertest ~ race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC, family=binomial(link="logit"), data=sample, subset = (!(prep_use_r %in% "Currently taking PrEP") & age %in% c(40:59)))
  nevertest_noraceXreg <- glm(nevertest ~ race_eth_m.reg + region.bKC, family=binomial(link="logit"), data=sample, subset = (!(prep_use_r %in% "Currently taking PrEP") & age %in% c(40:59)))
...
```{r}
  #Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$iti_int) & !(sample$prep_use_r %in% "Currently taking PrEP")])
  table(sample$age_cat[!is.na(sample$race_eth_m) & !is.na(sample$iti_int) & !(sample$prep_use_r %in% "Currently taking PrEP")])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$iti_int) & !(sample$prep_use_r %in% "Currently taking PrEP")])
...
```{r}
 #Figure out how to parameterize age
    iti_full_testage <- lm(iti_int ~ age_cat + race_eth_m.reg + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
...
```{r iti, eval=TRUE}
 #Define age categorization
  sample$age_cat_iti <- cut(sample$age, c(17, 24, 34, 59), labels=c("18-24", "25-34", "35-59")) 

 #Main effects
  iti_full <- lm(iti_int ~ age_cat_iti + race_eth_m.reg + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noregion <- lm(iti_int ~ age_cat_iti + race_eth_m.reg, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_norace <- lm(iti_int ~ age_cat_iti + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noage <- lm(iti_int ~ race_eth_m.reg + region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))

 #Interactions
  iti_full_withint <- lm(iti_int ~ age_cat_iti + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + age_cat_iti*region.bKC + race_eth_m.reg*age_cat_iti, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noraceXreg <- lm(iti_int ~ age_cat_iti + race_eth_m.reg + region.bKC + age_cat_iti*region.bKC + race_eth_m.reg*age_cat_iti, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noageXreg <- lm(iti_int ~ age_cat_iti + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + race_eth_m.reg*age_cat_iti, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
  iti_noraceXage <- lm(iti_int ~ age_cat_iti + race_eth_m.reg + region.bKC + age_cat_iti*region.bKC + race_eth_m.reg*region.bKC, data=sample, subset = !(prep_use_r %in% "Currently taking PrEP"))
...
From these coefficients, coital frequency appears to be higher for men aged 30-34, and then to decrease with age. In subsequent analyses, we will include age as a dummy variable with ages 40-59 in 10-year bins.
...
## Condom use
Data on condom use are restricted to dyads in which both the respondent and his partner were HIV-negative or of unknown status. This analysis includes data from all such partnerships, not just those that were ongoing. Because the model will explore the effect of decreased condom use in men on PrEP, we will further restrict this analysis to men who have never taken PrEP. We exclude men tho took PrEP in the past as well because `r 100*(sum(sample$monthsago_lastprep %in% c(0:12))/sum(!is.na(sample$monthsago_lastprep)))`% of men who discontinued took PrEP within the past 12 months, so their reported behavior may reflect behaviors while on PrEP.
...
```{r}
 ...
 #Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$condoms_main) & sample$prep_use_r %in% "Never taken PrEP"])
  table(sample$age_cat[!is.na(sample$race_eth_m) & !is.na(sample$condoms_main) & sample$prep_use_r %in% "Never taken PrEP"])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$condoms_main) & sample$prep_use_r %in% "Never taken PrEP"])
...
```{r}
 #Figure out how to parameterize age
    condoms_main_full_testage <- lm(condoms_main ~ age_cat + race_eth_m.reg + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
...
```{r condoms_main}
 ...
 #Main effects
  condoms_main_full <- lm(condoms_main ~ age_cat_alt + race_eth_m.reg + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_main_noregion <- lm(condoms_main ~ age_cat_alt + race_eth_m.reg, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_main_norace <- lm(condoms_main ~ age_cat_alt + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_main_noage <- lm(condoms_main ~ race_eth_m.reg + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")

 #Interactions
  condoms_main_full_withint <- lm(condoms_main ~ age_cat_alt + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + age_cat_alt*region.bKC + race_eth_m.reg*age_cat_alt, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_main_noraceXreg <- lm(condoms_main ~ age_cat_alt + race_eth_m.reg + region.bKC + age_cat_alt*region.bKC + race_eth_m.reg*age_cat_alt, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_main_noageXreg <- lm(condoms_main ~ age_cat_alt + race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC + race_eth_m.reg*age_cat_alt, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_main_noraceXage <- lm(condoms_main ~ age_cat_alt + race_eth_m.reg + region.bKC + age_cat_alt*region.bKC + race_eth_m.reg*region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
...
```{r}
 ...
 #Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$condoms_pers) & sample$prep_use_r %in% "Never taken PrEP"])
  table(sample$age_cat[!is.na(sample$race_eth_m) & !is.na(sample$condoms_pers) & sample$prep_use_r %in% "Never taken PrEP"])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$condoms_pers) & sample$prep_use_r %in% "Never taken PrEP"])
...
```{r}
 #Figure out how to parameterize age
    condoms_pers_full_testage <- lm(condoms_pers ~ age_cat + race_eth_m.reg + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
...
```{r condoms_pers}
 ...
 #Main effects
  condoms_pers_full <- lm(condoms_pers ~ race_eth_m.reg + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_pers_noregion <- lm(condoms_pers ~ race_eth_m.reg, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_pers_norace <- lm(condoms_pers ~ region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")

 #Interactions
  condoms_pers_full_withint <- lm(condoms_pers ~ race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_pers_noraceXreg <- lm(condoms_pers ~ race_eth_m.reg + region.bKC, data=sample, subset = prep_use_r %in% "Never taken PrEP")
...
```{r}
 ...
 #Look at the number of observations that are non-missing for this outcome and for model predictors
  table(sample$hbo[!is.na(sample$race_eth_m) & !is.na(sample$condoms_inst) & sample$prep_use_r %in% "Never taken PrEP"])
  table(sample$age_cat[!is.na(sample$race_eth_m) & !is.na(sample$condoms_inst) & sample$prep_use_r %in% "Never taken PrEP"])
  table(sample$region[!is.na(sample$race_eth_m) & !is.na(sample$condoms_inst) & sample$prep_use_r %in% "Never taken PrEP"])
...
```{r}
 #Figure out how to parameterize age
    condoms_inst_full_testage <- glm(condoms_inst ~ age_cat + race_eth_m.reg + region.bKC, family=binomial(link="logit"), data=sample, subset = prep_use_r %in% "Never taken PrEP")
    ...
    p2 <- ggplot(agecoefs_condoms_inst, aes(x=age_cat, y=coefs)) + geom_pointrange(aes(ymin=coefs - 1.96*se, ymax=coefs + 1.96*se)) + labs(x="Age groups", y="Adjusted coefficients and CI", title="Adjusted coefficients and 95% confidence intervals for age, dropping the 55-59 year-old group") + theme_title + ylim(-3, 5)
...
```{r condoms_inst}
 ...
 #Main effects
  condoms_inst_full <- glm(condoms_inst ~ race_eth_m.reg + region.bKC, family=binomial(link="logit"), data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_inst_noregion <- glm(condoms_inst ~ race_eth_m.reg, family=binomial(link="logit"), data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_inst_norace <- glm(condoms_inst ~ region.bKC, family=binomial(link="logit"), data=sample, subset = prep_use_r %in% "Never taken PrEP")

 #Interactions
  condoms_inst_full_withint <- glm(condoms_inst ~ race_eth_m.reg + region.bKC + race_eth_m.reg*region.bKC, family=binomial(link="logit"), data=sample, subset = prep_use_r %in% "Never taken PrEP")
  condoms_inst_noraceXreg <- glm(condoms_inst ~ race_eth_m.reg + region.bKC, family=binomial(link="logit"), data=sample, subset = prep_use_r %in% "Never taken PrEP")
...
## Summary tables
In these tables, "NA" indicates that age was not included in the model based on examination of the coefficients.
```{r summaries}
...
```{r plots_explore_ongoingmain}
    ...
    ggplot(mainXregion) + geom_pointrange(aes(x=region, y=percent, ymin=ci_lower, ymax=ci_upper)) + labs(x="Region", y="Percent", title="Percent of men with an ongoing main partner by region") + guides(size=guide_legend(title="Frequency")) + ylim(0, 1)  + plot_background + theme_title 
    ...
    ggplot(mainXrace) + geom_pointrange(aes(x=hbo, y=percent, ymin=ci_lower, ymax=ci_upper)) + labs(x="Race/ethnicity", y="Percent", title="Percent of men with an ongoing main partner by race/ethnicity") + ylim(0, 1) + plot_background + theme_title
...
```{r plots_explore_ongoingpers}
    ...
    ggplot(concurrent_mainpersXage) + geom_pointrange(aes(x=age_cat_alt, y=percent, ymin=ci_lower, ymax=ci_upper)) + labs(x="Age group", y="Percent", title="Percent of men with concurrent ongoing persistent partners by age") + guides(size=guide_legend(title="Frequency")) + ylim(0, 0.6)  + plot_background + theme_title 
```
__Decision__: <span style="color:red"> Fill me in </span>
...
To explore this further, we look at the proportion of men in each age group that fall in each of the quartiles of the distribution of instantaneous partnerships identified in section \@ref(rateinst_explore). The error bars indicate the 95% confidence interval on the point estimates.
...
In the next plots, we will look to see whether the distribution by age differs by momentary degree. <span style="color:red"> These plots exclude the four outliers identified above </span>. 
```{r plots_explore_rateinst_4}
#Rate of instantaneous partnerships by age and momentary degree
    p_rateinstXage_main <- ggplot(sample[!is.na(sample$rate_inst) & sample$rate_inst <0.25 & !is.na(sample$degree_main),], aes(x=age, y=rate_inst)) + geom_point() + geom_smooth(method="loess") + facet_grid(.~degree_main) + labs(x="Ego age", y="Rate of instantaneous partnerships", title="Rate of instantaneous partnerships by ego age and main partnership status") + plot_background + theme_title
    p_rateinstXage_pers <- ggplot(sample[!is.na(sample$rate_inst) & sample$rate_inst <0.25 & !is.na(sample$anypers),], aes(x=age, y=rate_inst)) + geom_point() + geom_smooth(method="loess") + facet_grid(.~anypers) + labs(x="Ego age", y="Rate of instantaneous partnerships", title="Rate of instantaneous partnerships by ego age and persistent partnership status") + plot_background + theme_title

    multiplot(p_rateinstXage_main, p_rateinstXage_pers, cols=1)
```
These plots do not suggest any clear differences in the distribution of instantaneous partnerships by age. The plots below examine whether the quantile distribution by age varies by main and persistent partnership status. <span style="color:red">Red</span> indicates zero instantaneous partnerships, <span style="color:orange">orange</span> indicates a low rate, <span style="color:blue">blue</span> indicates a medium rate, and <span style="color:green">green</span> indicates a high rate.
```{r plotsexplore_rateinst_5}
    ...
    multiplot(p_quartile_nomain, p_quartile_main, p_quartile_nopers, p_quartile_pers, cols=2)
...
```{r plots_explore_agemixing_1}
    ...
    age_diff <- ggplot(sample[!is.na(sample$agediff) & !is.na(sample$ptype_adiff),], aes(x=age, y=agediff)) + geom_point() + geom_smooth(method="loess") + facet_grid(ptype_adiff ~.) + labs(x="Ego age", y="Absolute value of the difference between ego and alter ages", title = "Age mixing as the difference between ego and alter ages") + theme_title + plot_background + theme_title
    
    multiplot(age_scatter, age_diff, cols=2)
    
```

To inform how to parameterize age differences, plot \@ref(fig:plots_explore_agemixing_2) shows the absolute difference between the square root of ego and alter ages by ego ego, and plot \@ref(fig:plots_explore_agemixing_3) shows the absolute difference between the cube root of ego and alter ages by ego ego.
```{r plots_explore_agemixing_2}

#Age mixing represented as square root of age differences
    ggplot(sample[!is.na(sample$sqrt_agediff) & !is.na(sample$ptype_adiff),], aes(x=age, y=sqrt_agediff)) + geom_point() + geom_smooth(method="loess") + facet_grid(ptype_adiff ~.) + labs(x="Ego age", y="Absolute value of the difference between the sqrt of ego and the sqrt of alter ages", title = "Age mixing as the difference between the square root of ego and alter ages") + theme_title
...
#Age mixing by age and race 
    ...
    agemixingXrace <- sample %>% filter(!is.na(agediff) & !is.na(ptype_adiff)) %>% group_by(ptype_adiff, age_cat, race_eth_m) %>% summarise(freq= n(), mean = mean(agediff))
    ggplot(agemixingXrace) + geom_point(aes(x=age_cat, y=mean, size=freq, colour=race_eth_m)) + facet_grid(ptype_adiff ~.) + geom_hline(yintercept=0) + labs(x="Ego age group", y="Mean difference between ego and partner age (absolute value)", title="Age differences by race and age category") + theme_title + plot_background

```

__Decision__: To capture the age heterogeneity in age mixing, we need to decide how to parameterize the age difference. From plots \@ref(fig:plots_explore_agemixing_2) and \@ref(fig:plots_explore_agemixing_3), taking the difference between the square root of ego and alter ages reduces the heterogeneity by age, but does not fully account for differences by ego age in the age difference with persistent partnerships. Taking the cube root does not lead to a noticeable improvement. <span style="color:red"> How to proceed?</span>

In the regression analyses, age mixing differed by race/ethnicity only among main partnerships. In the plot, the patterns for non-Hispanic other and Hispanic respondents appear similar. Black men ages 18-24 report partners who are, on average, further in age from themselves than the other racial/ethnic groups. We see this for persistent partnerships as well. However, the numbers are small: `r sum(!is.na(sample$mrp_ageinyears_approx) & sample$race_eth_m %in% "Black" & sample$mrp_type_ongoing %in% "Main" & sample$age_cat %in% "18-24")` black respondents provided data on the age of main partners and only `r sum(!is.na(sample$mrp_ageinyears_approx) & sample$race_eth_m %in% "Black" & sample$mrp_type_ongoing %in% "Persistent" & sample$age_cat %in% "18-24")` provided data on the age of persistent partners. As such, these patterns should be interpreted with caution. <span style="color:red">To capture potential racial ethnic differences, we will represent heterogeneity in age mixing in both main and persistent partnerships by race, but we will not attempt to represent an interaction between race and age on patterns of age mixing.</span>
...
```{r plotsexplore_nevertest}
    sample$nevertest <- ifelse(sample$evertest_r %in% 1, 0, ifelse(sample$evertest_r %in% 0, 1, NA))
...
```
__Decision__: <span style="colour:red">This plot indicates that the proportion of men who have never tested for HIV </span>

### Intertest interval by age
The first figure below looks at the mean and median intertest intervals by age group. The second shows only the median to allow for a closer examination of the variability by age.

```{r plotsexplore_iti}

#Intertest interval by age 
    itiXage <- sample %>% filter(!is.na(iti_int) & prep_use_r !="Currently taking PrEP") %>% group_by(age) %>% summarise(freq=n(), median=median(iti_int, na.rm=TRUE), mean=mean(iti_int, na.rm=TRUE))
...
#Condom use in main partnerships by ego age
   p_condoms_main <- ggplot(sample[!is.na(sample$condoms_main) & sample$prep_use_r %in% "Never taken PrEP", ], aes(x=age, y=condoms_main)) + geom_point() + labs(x="Ego age", y="Probability of condom use", title = "Probability of condom use in main partnerships by respondent age") + plot_background + theme_title
   condoms_mainXage <- sample %>% filter(!is.na(condoms_main) & prep_use_r %in% "Never taken PrEP") %>% group_by(age_cat) %>% summarise(freq=n(), mean=mean(condoms_main))
...
#Condom use in persistent partnerships by ego age
 p_condoms_pers <- ggplot(sample[!is.na(sample$condoms_pers) & sample$prep_use_r %in% "Never taken PrEP",], aes(x=age, y=condoms_pers)) + geom_point() + labs(x="Ego age", y="Probability of condom use", title = "Probability of condom use in persistent partnerships by respondent age") + plot_background + theme_title
   condoms_persXage <- sample %>% filter(!is.na(condoms_pers) & prep_use_r %in% "Never taken PrEP") %>% group_by(age_cat) %>% summarise(freq=n(), mean=mean(condoms_pers))
...
#Condom use in instantaneous partnerships by ego age
   condoms_instXage <- sample %>% filter(!is.na(condoms_inst) & prep_use_r %in% "Never taken PrEP") %>% group_by(age_cat) %>% summarise(freq=n(), mean=mean(condoms_inst))
