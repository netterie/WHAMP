...
#-----------------------------------------------------------------------------------
    ...
    load(file="Data/iti_coefs.Rdata")
...
# Add rows for each parameter
 ...
 # Condom use
  epi_params <- param_table(epi_params, "condoms.main.YY.prob", "Probability of condom use in Young-Young (18-34) dyads", round(condoms_prob$condoms_main_prob$condoms_main[condoms_prob$condoms_main_prob$dyad_age_bin %in% "Young-Young"], 4), NA, "WHPP survey, reweighted to Washington demographics for HIV-negative/undiagnosed men, restricted to HIV-neg/unknown and non-PrEP-using dyads")
  epi_params <- param_table(epi_params, "condoms.main.other.prob", "Probability of condom use in other age dyads", round(condoms_prob$condoms_main_prob$condoms_main[condoms_prob$condoms_main_prob$dyad_age_bin %in% "Other ages"], 4), NA, "WHPP survey, reweighted to Washington demographics for HIV-negative/undiagnosed men, restricted to HIV-neg/unknown and non-PrEP-using dyads")
  epi_params <- param_table(epi_params, "cond.pi.always.prob", "Fraction of men in persistent and/or instantaneous partnerships who always use condoms in those partnerships", round(condoms_prob$condoms_always_nonmain[1], 4), NA, "WHPP survey, reweighted to Washington demographics for HIV-negative/undiagnosed men, restricted to HIV-neg/unknown and non-PrEP-using dyads")
 ...
 # Circumcision
  ...
  # HIV testing
  epi_params <- param_table(epi_params, "iti.coefs", "GLM coefficients for median intertest interval (intercept, centered age, centered age squared)", paste(lm_itiXage_coefs, collapse=", "), NA, "WHPP survey, reweighted to Washington demographics for HIV-negative/undiagnosed men, restricted to PrEP non-users")
  
# Make a kable
rownames(epi_params) <- NULL
kable(epi_params, caption="Epidemic model parameter values and sources", digits=c(0, 0, 4, 4, 0)) %>% kable_styling(full_width=F, position="center") %>% column_spec(1, width = "5cm") %>% column_spec(2, width = "10cm") %>% column_spec(3, width = "5cm") %>% column_spec(4, width = "8cm") %>% column_spec(5, width = "20cm") %>% group_rows("Coital frequency", 1,2) %>% group_rows("Condom use", 3,7) %>% group_rows("Circumcision", 8,8) %>% group_rows("HIV testing", 9,9)
