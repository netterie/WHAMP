...
# Add rows for each parameter
 ...
 #Network composition
  ...
  parameters <- param_table(parameters, "inst.region", "Mean rate of instantaneous partnerships by region (EW, KC, OW)",  paste(round(meansXregion$Mean, 6), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
...
kable(parameters, caption="Parameter values and sources", digits=c(0, 0, 4, 4, 0)) %>% kable_styling(full_width=F, position="center") %>% column_spec(1, width = "5cm") %>% column_spec(2, width = "10cm") %>% column_spec(3, width = "5cm") %>% column_spec(4, width = "8cm") %>% column_spec(5, width = "20cm") %>% group_rows("Model specifications", 1,1) %>% group_rows("Network composition",2,11) %>% group_rows("Age-specific mortality", 12,14) %>%  group_rows("Degree distribution by nodal attribute", 15,21) %>% group_rows("Rate of instantaneous partnerships", 22,26) %>% group_rows("Partnership duration", 27,28) %>% group_rows("Age mixing", 29,29) %>% group_rows("Racial/ethnic mixing", 30,32) %>% group_rows("Regional mixing", 33,33) %>% group_rows("Sex role", 34,34)
```
