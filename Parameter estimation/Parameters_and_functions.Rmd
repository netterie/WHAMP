...
#-----------------------------------------------------------------------------------
    ...
    load(file="Data/rate_inst_hiv.Rdata")
...
# Add rows for each parameter
 ...
 #Network composition
  ...
  parameters <- param_table(parameters, "degm.region", "Main degree distribution by region (EW, KC, OW)", paste(as.vector(t(round(degree$degm.region$proportion, 4))), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "degp.region", "Persistent degree distribution by region (EW, KC, OW)", paste(as.vector(t(round(degree$degp.region$proportion, 4))), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "deg.mp.negunk", "Degree distribution for HIV-negative and undiagnosed men", paste(as.vector(t(round(degree$deg.mp.hivneg[1:2,1:3], 4))), collapse=", "), NA, "WHPP survey, reweighted to Washington demographics")
  parameters <- param_table(parameters, "deg.mp.dxpos", "Degree distribution for diagnosed HIV-positive men", paste(as.vector(t(round(degree$deg.mp.hivpos[1:2,1:3], 4))), collapse=", "), NA, "ART-Net survey national data from HIV+ men, reweighted to age and race distribution of MSM living with diagnosed HIV in Washington")
  ...
  parameters <- param_table(parameters, "qnts.18to49", "Mean rate of instantaneous partnerships by quartile among men aged 18 to 49", paste(round(avg.qrtmeans.18to49, 6), collapse=", "), NA, "WHPP survey and national ART-Net data from HIV-positive respondents, reweighted to Washington demographics")
  parameters <- param_table(parameters, "qnts.50to59", "Mean rate of instantaneous partnerships by quartile among men aged 50 to 59",  paste(round(avg.qrtmeans.50to59, 6), collapse=", "), NA, "WHPP survey and national ART-Net data from HIV-positive respondents, reweighted to Washington demographics")
  parameters <- param_table(parameters, "inst.bho", "Mean rate of instantaneous partnerships by race/ethnicity (B, H, O)",  paste(round(meansXhbo$Mean, 6), collapse=", "), NA, "WHPP survey and national ART-Net data from HIV-positive respondents, reweighted to Washington demographics")
  parameters <- param_table(parameters, "inst.region", "Distribution of instantaneous partnerships by region (EW, KC, OW)",  paste(round(inst.dist.region$proportion, 6), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "inst.hiv", "Mean rate of instantaneous partnerships by HIV status (HIV-, HIV+)", paste(round(meansXhiv$Mean, 6), collapse=", "), NA, "WHPP survey and national ART-Net data from HIV-positive respondents, reweighted to Washington demographics")
...
kable(parameters, caption="Parameter values and sources", digits=c(0, 0, 4, 4, 0)) %>% kable_styling(full_width=F, position="center") %>% column_spec(1, width = "5cm") %>% column_spec(2, width = "10cm") %>% column_spec(3, width = "5cm") %>% column_spec(4, width = "8cm") %>% column_spec(5, width = "20cm") %>% group_rows("Model specifications", 1,1) %>% group_rows("Network composition",2,11) %>% group_rows("Age-specific mortality", 12,14) %>%  group_rows("Degree distribution by nodal attribute", 15,22) %>% group_rows("Rate of instantaneous partnerships", 23,28) %>% group_rows("Partnership duration", 29,30) %>% group_rows("Age mixing", 31,31) %>% group_rows("Racial/ethnic mixing", 32,34) %>% group_rows("Regional mixing", 35,35) %>% group_rows("Sex role", 36,36)
```
