...
kable(parameters, caption="Parameter values and sources", digits=c(0, 0, 4, 4, 0)) %>% kable_styling(full_width=F, position="center") %>% column_spec(1, width = "5cm") %>% column_spec(2, width = "10cm") %>% column_spec(3, width = "5cm") %>% column_spec(4, width = "8cm") %>% column_spec(5, width = "20cm") %>% group_rows("Model specifications", 1,1) %>% group_rows("Network composition",2,11) %>% group_rows("Degree distribution by nodal attribute", 12,18) %>% group_rows("Rate of instantaneous partnerships", 19,23) %>% group_rows("Partnership duration", 24,25) %>% group_rows("Age mixing", 26,26) %>% group_rows("Racial/ethnic mixing", 27,29) %>% group_rows("Regional mixing", 30,30) %>% group_rows("Sex role", 31,31) %>% group_rows("Age-specific mortality", 32,34)
```
