...
#-----------------------------------------------------------------------------------
    ...
    load(file="Data/nodefactor_mp.Rdata")
    load(file="Data/rate_inst_mdeg.Rdata")
...
# Add rows for each parameter
 ...
 #Network composition
  ...
  parameters <- param_table(parameters, "deg.mp.H", "Degree distribution for Hispanic men", paste(as.vector(t(round(degree$deg.mp.H[1:2,1:3], 4))), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "deg.mp.B", "Degree distribution for black men", paste(as.vector(t(degree$deg.mp.B[1:2,1:3])), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "deg.mp.O", "Degree distribution for other race/ethnicity men", paste(as.vector(t(round(degree$deg.mp.O[1:2,1:3], 4))), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  ...
  parameters <- param_table(parameters, "deg.mp.otherages", "Degree distribution for men aged 18-39 and 50-59", paste(as.vector(t(round(degree$deg.mp.otherages[1:2,1:3], 4))), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "deg.mp.40to49", "Degree distribution for men aged 40-49", paste(as.vector(t(round(degree$deg.mp.40to49[1:2,1:3], 4))), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
  parameters <- param_table(parameters, "prop.main.hbo", "Proportion of nodes in main partnerships that are Hispanic, Black, and Other", paste(as.vector(round(nodefactor_mp$prop.inmainXrace, 4)), collapse=", "), NA, "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM. EVENTUALLY REPLACE USING DATA ON MIXING BY RACE FROM MMP AND FROM THE WHPP SAMPLE REWEIGHTED TO NEG/UNK TOTALS. (BUT WILL NEED TO USE DATA FROM WHPP REWEIGHTED TO ALL FOR DEGREE)")
  parameters <- param_table(parameters, "prop.main.region", "Proportion of nodes in main partnerships that are in King County, other counties in western WA, and eastern WA", paste(as.vector(round(nodefactor_mp$prop.inmainXregion, 4)), collapse=", "), "Consider exploring scenarios with cross-regional mixing", "WHPP survey with assumptions about alters' region")
  parameters <- param_table(parameters, "prop.pers.age", "Proportion of nodes in persistent partnerships that are ages 18-39 or 50-59 vs. ages 40-49 (note this double-counts nodes with concurrent pships)", "NULL - not included in the base model", paste(as.vector(round(nodefactor_mp$prop.inpersXage, 4)), collapse=", "), "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM. EVENTUALLY REPLACE USING DATA ON MIXING BY AGE FROM MMP AND FROM THE WHPP SAMPLE REWEIGHTED TO NEG/UNK TOTALS. WILL NEED TO USE DATA FROM WHPP REWEIGHTED TO ALL FOR DEGREE")
