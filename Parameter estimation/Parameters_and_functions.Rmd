...
# Add rows for each parameter
 ...
 #Network composition
  ...
  parameters <- param_table(parameters, "deg.mp.B", "Degree distribution for black men. Base values after adjustment to align with balanced mixing matrix, alternate values without adjustment", paste(as.vector(t(degree$deg.mp.B.adj[1:2,1:3])), collapse=", "), paste(as.vector(t(degree$deg.mp.B[1:2,1:3])), collapse=", "), "WHPP survey, assumed to apply to HIV-negative and HIV-positive MSM")
...
kable(parameters, caption="Parameter values and sources", digits=c(0, 0, 4, 4, 0)) %>% kable_styling(full_width=F, position="center") %>% column_spec(1, width = "5cm") %>% column_spec(2, width = "10cm") %>% column_spec(3, width = "5cm") %>% column_spec(4, width = "8cm") %>% column_spec(5, width = "20cm") %>% group_rows("Model specifications", 1,1) %>% group_rows("Network composition",2,11) %>% group_rows("Degree distribution by nodal attribute", 12,18) %>% group_rows("Rate of instantaneous partnerships", 19,21) %>% group_rows("Partnership duration", 22,23) %>% group_rows("Age mixing", 24,24) %>% group_rows("Racial/ethnic mixing", 25,27) %>% group_rows("Regional mixing", 28,28) %>% group_rows("Sex role", 29,29) %>% group_rows("Age-specific mortality", 30,32)
```
