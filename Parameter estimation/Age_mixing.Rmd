...
#Conditional row probabilities
  ...
  #N by age in reweighted sample -- scaling to the total N assumes that those with missing data on degree or who didn't provide the age of their most recent partner are missing at random
      n <- 10000
      agestr <- as.vector(prop.table(svytable(~age_cat, sample_rake_all, round=FALSE)))
      n.by.age <- c(round(n*agestr[1]), round(n*agestr[2]), round(n*agestr[3]), round(n*agestr[4]), round(n*agestr[5]), round(n*agestr[6]),
                    round(n*agestr[7]))
      n.by.age[8] <- n - sum(n.by.age[1:7])
      age <- c(sample(seq(18, 24, 1), n.by.age[1], TRUE),
             sample(seq(25, 29, 1), n.by.age[2], TRUE),
             sample(seq(30, 34, 1), n.by.age[3], TRUE),
             sample(seq(35, 39, 1), n.by.age[4], TRUE),
             sample(seq(40, 44, 1), n.by.age[5], TRUE),
             sample(seq(45, 49, 1), n.by.age[6], TRUE),
             sample(seq(50, 54, 1), n.by.age[7], TRUE),
             sample(seq(55, 59, 1), n.by.age[8], TRUE))
      nXage <- table(age)
