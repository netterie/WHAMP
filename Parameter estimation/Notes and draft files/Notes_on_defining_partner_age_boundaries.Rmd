...
# Restrict all ages dataset to age range and make other modifications. Don't use the "sample" dataset b/c that excludes men with a partner outside the age range.
...
This file looks at the characteristics of men whose most recent partner was outside of the modeled age range (<18 or >59) and whether these men differ in any meaningful way from other respondents. In either case, we will exclude these men from calculation of any parameter related to the network model. If they are differnet in any meaningful way, we will still drop them, but flag this as something to potentially look into more in the future.

The table below shows the number of men who reported a most recent partner outside of the age range, by partner type
```{r}
...
Next we'll look to see whether men who reported partners outside of the age range differ in any important way from the rest of the sample_incl.mrp.oor.
```{r, echo=TRUE}
# Ego age
table(sample_incl.mrp.oor$age_cat, sample_incl.mrp.oor$mrp_oor)
```

Because men whose partners are out of range are more likely to be close to the age cut-offs, we will adjust for ego age in comparisons for other variables.
