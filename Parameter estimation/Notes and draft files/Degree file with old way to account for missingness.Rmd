...
To calculate the momentary degree distribution, we use data from the re-weighted WHPP survey, as we do not have data specific to HIV-positive men. To account for differential missingness on questions relating to current main and casual partnership counts, we define separate degree distributions for each combination of attributes (race, region and age) and weight each by the inverse of the proportion of people in the group that was non-missing on degree, and then define overall distributions as the sum of these adjusted distributions. This is esssentially the same as imputing degree for those with missing data, and assumes non-differential missingness. It ensures that degree distributions by subgroup are consistent and imply the same total number of partnerships (see [issue 39](https://github.com/statnet/WHAMP/issues/39)).

```{r degmatrix_adjustment, include=FALSE}
## Define a variable that indicates race/ethnicity, region, and age group 
sample_rake_all$variables$attrgroup <- rep(NA, dim(sample_rake_all)[1])
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "King County" & sample_rake_all$variables$hbo %in% "Hispanic" & sample_rake_all$variables$age40to49 %in% 0] <- "KC.H.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "King County" & sample_rake_all$variables$hbo %in% "Hispanic" & sample_rake_all$variables$age40to49 %in% 1] <- "KC.H.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "King County" & sample_rake_all$variables$hbo %in% "Black" & sample_rake_all$variables$age40to49 %in% 0] <- "KC.B.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "King County" & sample_rake_all$variables$hbo %in% "Black" & sample_rake_all$variables$age40to49 %in% 1] <- "KC.B.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "King County" & sample_rake_all$variables$hbo %in% "Other" & sample_rake_all$variables$age40to49 %in% 0] <- "KC.O.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "King County" & sample_rake_all$variables$hbo %in% "Other" & sample_rake_all$variables$age40to49 %in% 1] <- "KC.O.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Western WA" & sample_rake_all$variables$hbo %in% "Hispanic" & sample_rake_all$variables$age40to49 %in% 0] <- "OW.H.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Western WA" & sample_rake_all$variables$hbo %in% "Hispanic" & sample_rake_all$variables$age40to49 %in% 1] <- "OW.H.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Western WA" & sample_rake_all$variables$hbo %in% "Black" & sample_rake_all$variables$age40to49 %in% 0] <- "OW.B.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Western WA" & sample_rake_all$variables$hbo %in% "Black" & sample_rake_all$variables$age40to49 %in% 1] <- "OW.B.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Western WA" & sample_rake_all$variables$hbo %in% "Other" & sample_rake_all$variables$age40to49 %in% 0] <- "OW.O.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Western WA" & sample_rake_all$variables$hbo %in% "Other" & sample_rake_all$variables$age40to49 %in% 1] <- "OW.O.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Eastern WA" & sample_rake_all$variables$hbo %in% "Hispanic" & sample_rake_all$variables$age40to49 %in% 0] <- "EW.H.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Eastern WA" & sample_rake_all$variables$hbo %in% "Hispanic" & sample_rake_all$variables$age40to49 %in% 1] <- "EW.H.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Eastern WA" & sample_rake_all$variables$hbo %in% "Black" & sample_rake_all$variables$age40to49 %in% 0] <- "EW.B.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Eastern WA" & sample_rake_all$variables$hbo %in% "Black" & sample_rake_all$variables$age40to49 %in% 1] <- "EW.B.40to49"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Eastern WA" & sample_rake_all$variables$hbo %in% "Other" & sample_rake_all$variables$age40to49 %in% 0] <- "EW.O.otherages"
sample_rake_all$variables$attrgroup[sample_rake_all$variables$region %in% "Eastern WA" & sample_rake_all$variables$hbo %in% "Other" & sample_rake_all$variables$age40to49 %in% 1] <- "EW.O.40to49"
sample_rake_all$variables$attrgroup <- factor(sample_rake_all$variables$attrgroup, levels = c("KC.H.otherages", "KC.H.40to49", "KC.B.otherages", "KC.B.40to49", "KC.O.otherages", "KC.O.40to49", "OW.H.otherages", "OW.H.40to49", "OW.B.otherages", "OW.B.40to49", "OW.O.otherages", "OW.O.40to49", "EW.H.otherages", "EW.H.40to49", "EW.B.otherages", "EW.B.40to49", "EW.O.otherages", "EW.O.40to49"))
    
## Proportion non-missing from each attribute group----
prop.nonmissing <- (svytable(~attrgroup, sample_rake_all[!is.na(sample_rake_all$variables$degree_main) &
                                                                !is.na(sample_rake_all$variables$degreecat_cas)],
                             round=FALSE)) / (svytable(~attrgroup, sample_rake_all, round=FALSE))

## Re-weighted degree matrices by race/ethnicity, region, and age (40-49 vs other)----
deg.mp.KC.H.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "KC.H.otherages"], round=FALSE)*(1/prop.nonmissing["KC.H.otherages"])
deg.mp.KC.H.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "KC.H.40to49"], round=FALSE)*(1/prop.nonmissing["KC.H.40to49"])
deg.mp.KC.B.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "KC.B.otherages"], round=FALSE)*(1/prop.nonmissing["KC.B.otherages"])
deg.mp.KC.B.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "KC.B.40to49"], round=FALSE)*(1/prop.nonmissing["KC.B.40to49"])
deg.mp.KC.O.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "KC.O.otherages"], round=FALSE)*(1/prop.nonmissing["KC.O.otherages"])
deg.mp.KC.O.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "KC.O.40to49"], round=FALSE)*(1/prop.nonmissing["KC.O.40to49"])

deg.mp.OW.H.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "OW.H.otherages"], round=FALSE)*(1/prop.nonmissing["OW.H.otherages"])
deg.mp.OW.H.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "OW.H.40to49"], round=FALSE)*(1/prop.nonmissing["OW.H.40to49"])
deg.mp.OW.B.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "OW.B.otherages"], round=FALSE)*(1/prop.nonmissing["OW.B.otherages"])
deg.mp.OW.B.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "OW.B.40to49"], round=FALSE)*(1/prop.nonmissing["OW.B.40to49"])
deg.mp.OW.O.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "OW.O.otherages"], round=FALSE)*(1/prop.nonmissing["OW.O.otherages"])
deg.mp.OW.O.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "OW.O.40to49"], round=FALSE)*(1/prop.nonmissing["OW.O.40to49"])

deg.mp.EW.H.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "EW.H.otherages"], round=FALSE)*(1/prop.nonmissing["EW.H.otherages"])
deg.mp.EW.H.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "EW.H.40to49"], round=FALSE)*(1/prop.nonmissing["EW.H.40to49"])
deg.mp.EW.B.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "EW.B.otherages"], round=FALSE)*(1/prop.nonmissing["EW.B.otherages"])
deg.mp.EW.B.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "EW.B.40to49"], round=FALSE)*(1/prop.nonmissing["EW.B.40to49"])
deg.mp.EW.O.otherages.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "EW.O.otherages"], round=FALSE)*(1/prop.nonmissing["EW.O.otherages"])
deg.mp.EW.O.40to49.rwt <- svytable(~degree_main + degreecat_cas, sample_rake_all[sample_rake_all$variables$attrgroup %in% "EW.O.40to49"], round=FALSE)*(1/prop.nonmissing["EW.O.40to49"])

## Weighted distributions for overall, by race/eth, region, and age----
deg.mp.H <- deg.mp.KC.H.otherages.rwt + 
    deg.mp.KC.H.40to49.rwt + 
    deg.mp.OW.H.otherages.rwt +
    deg.mp.OW.H.40to49.rwt + 
    deg.mp.EW.H.otherages.rwt +
    deg.mp.EW.H.40to49.rwt

deg.mp.B <- deg.mp.KC.B.otherages.rwt + 
    deg.mp.KC.B.40to49.rwt + 
    deg.mp.OW.B.otherages.rwt +
    deg.mp.OW.B.40to49.rwt + 
    deg.mp.EW.B.otherages.rwt

deg.mp.O <- deg.mp.KC.O.otherages.rwt +
    deg.mp.KC.O.40to49.rwt + 
    deg.mp.OW.O.otherages.rwt +
    deg.mp.OW.O.40to49.rwt + 
    deg.mp.EW.O.otherages.rwt +
    deg.mp.EW.O.40to49.rwt

deg.mp.KC <- deg.mp.KC.H.otherages.rwt +
    deg.mp.KC.H.40to49.rwt + 
    deg.mp.KC.B.otherages.rwt +
    deg.mp.KC.B.40to49.rwt + 
    deg.mp.KC.O.otherages.rwt +
    deg.mp.KC.O.40to49.rwt
    
deg.mp.OW <- deg.mp.OW.H.otherages.rwt + 
    deg.mp.OW.H.40to49.rwt + 
    deg.mp.OW.B.otherages.rwt +
    deg.mp.OW.B.40to49.rwt + 
    deg.mp.OW.O.otherages.rwt +
...
deg.mp.EW <- deg.mp.EW.H.otherages.rwt +
    deg.mp.EW.H.40to49.rwt + 
    deg.mp.EW.B.otherages.rwt +
    deg.mp.EW.O.otherages.rwt +
    deg.mp.EW.O.40to49.rwt 

deg.mp.40to49 <- deg.mp.KC.H.40to49.rwt + 
    deg.mp.KC.B.40to49.rwt + 
    deg.mp.KC.O.40to49.rwt + 
    deg.mp.OW.H.40to49.rwt + 
    deg.mp.OW.B.40to49.rwt + 
...
deg.mp.otherages <- deg.mp.KC.H.otherages.rwt + 
    deg.mp.KC.B.otherages.rwt + 
    deg.mp.KC.O.otherages.rwt + 
    deg.mp.OW.H.otherages.rwt + 
    deg.mp.OW.B.otherages.rwt + 
...
deg.mp <- prop.table(deg.mp.H + deg.mp.B + deg.mp.O)

#CHECK TO MAKE SURE THIS WORKED
# ## Compare estimates of overall degree distribution from averages of distributions by race, region, and age----
# deg.mp1 <- round(prop.table(deg.mp.H + deg.mp.B + deg.mp.O), 4)  
# deg.mp2 <- round(prop.table(deg.mp.KC + deg.mp.OW + deg.mp.EW), 4)
# deg.mp3 <- round(prop.table(deg.mp.40to49 + deg.mp.otherages), 4)
# deg.mp1 == deg.mp2 
# deg.mp1 == deg.mp3
# 
# ## Implied number of partnerships----
# num.edges.H <- prop.table(deg.mp.H)*svytable(~hbo, sample_rake_all, round=FALSE)[1]
# num.edges.B <- prop.table(deg.mp.B)*svytable(~hbo, sample_rake_all, round=FALSE)[2]
# num.edges.O <- prop.table(deg.mp.O)*svytable(~hbo, sample_rake_all, round=FALSE)[3]
# num.edges1 <- prop.table(deg.mp.H + deg.mp.B + deg.mp.O)*sum(weights(sample_rake_all))
# ### Main edges
...
# sum(num.edges1[,2:3])
# 
# num.edges.KC <- prop.table(deg.mp.KC)*svytable(~region, sample_rake_all, round=FALSE)[1]
# num.edges.OW <- prop.table(deg.mp.OW)*svytable(~region, sample_rake_all, round=FALSE)[2]
# num.edges.EW <- prop.table(deg.mp.EW)*svytable(~region, sample_rake_all, round=FALSE)[3]
# num.edges2 <- prop.table(deg.mp.KC + deg.mp.OW + deg.mp.EW)*sum(weights(sample_rake_all))
# ### Main edges
...
# sum(num.edges2[,2:3])
# 
# num.edges.40to49 <- prop.table(deg.mp.40to49)*svytable(~age40to49, sample_rake_all, round=FALSE)[2]
# num.edges.otherages <- prop.table(deg.mp.otherages)*svytable(~age40to49, sample_rake_all, round=FALSE)[1]
# num.edges3 <- prop.table(deg.mp.40to49 + deg.mp.otherages)*sum(weights(sample_rake_all))
# ### Main edges
...
```{r degmatrix_KC}
# Add margins to degree distribution table
deg.mp.KC <- prop.table(deg.mp.KC)
...
```{r degmatrix_OW}
# Add margins to degree distribution table
deg.mp.OW <- prop.table(deg.mp.OW)
...
```{r degmatrix_EW}
# Add margins to degree distribution table
deg.mp.EW <- prop.table(deg.mp.EW)
...
# Add margins to degree distribution table
deg.mp.otherages <- prop.table(deg.mp.otherages)
...
```{r degmatrix_40to49}
# Add margins to degree distribution table
deg.mp.40to49 <- prop.table(deg.mp.40to49)
