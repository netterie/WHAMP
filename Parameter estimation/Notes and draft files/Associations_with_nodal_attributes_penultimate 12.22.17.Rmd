...
#Age mixing by age and race 
    ...
    ggplot(agemixingXrace) + geom_point(aes(x=age_cat, y=mean, size=freq, colour=race_eth_m)) + facet_grid(ptype_adiff ~.) + labs(x="Ego age group", y="Mean difference between ego and alter age (absolute value)", title="Age differences by race/ethnicity and age category") + theme_title + plot_background
...
#Age mixing by age and region 
    ...
    agemixingXregion <- sample %>% filter(!is.na(agediff) & !is.na(ptype_adiff)) %>% group_by(ptype_adiff, age_cat, region) %>% summarise(freq= n(), mean = mean(agediff))
    ggplot(agemixingXregion) + geom_point(aes(x=age_cat, y=mean, size=freq, colour=region)) + facet_grid(ptype_adiff ~.) + labs(x="Ego age group", y="Mean difference between ego and alter age (absolute value)", title="Age differences by region and age category") + theme_title + plot_background
...
```{r}
#Define age centered, age centered squared, and age centered cubed terms for the no PrEP dataframe
sample_p12noprep_noalterprep$age_centered <- sample_p12noprep_noalterprep$age - mean(sample_p12noprep_noalterprep$age, na.rm=TRUE)
sample_p12noprep_noalterprep$age_cent_sq <- sample_p12noprep_noalterprep$age_centered^2
sample_p12noprep_noalterprep$age_cent_cube <- sample_p12noprep_noalterprep$age_centered^3
...
condoms_mainXageinyrs <- sample_p12noprep_noalterprep %>% filter(!is.na(condoms_main)) %>% group_by(age) %>% summarise(freq=n(), mean=mean(condoms_main))

#Fit linear model to the mean condom use, adjusting for centered age and centered age squared
lm_condoms_mainXage2 <- lm(condoms_main ~ age_centered + age_cent_sq, data=sample_p12noprep_noalterprep)

#Fit linear model to the mean condom use, adjusting for centered age, centered age squared, and centered age cubed
lm_condoms_mainXage3 <- lm(condoms_main ~ age_centered + age_cent_sq + age_cent_cube, data=sample_p12noprep_noalterprep)

#Create a data frame with predictions from the models and age
condoms_main_pred_age2 <- cbind.data.frame(prediction = predict(lm_condoms_mainXage2, list(age_centered = sample_p12noprep_noalterprep$age_centered, age_cent_sq = (sample_p12noprep_noalterprep$age_centered)^2)), age = sample_p12noprep_noalterprep$age)

condoms_main_pred_age3 <- cbind.data.frame(prediction = predict(lm_condoms_mainXage3, list(age_centered = sample_p12noprep_noalterprep$age_centered, age_cent_sq = (sample_p12noprep_noalterprep$age_centered)^2, age_cent_cube = (sample_p12noprep_noalterprep$age_centered)^3)), age = sample_p12noprep_noalterprep$age)

#Plot
 ggplot() + geom_point(data=condoms_mainXageinyrs, aes(x=age, y=mean, size=freq)) + geom_line(data=condoms_main_pred_age2, aes(x=age, y=prediction, color="a")) + geom_line(data=condoms_main_pred_age3, aes(x=age, y=prediction, color="b")) + scale_colour_manual(name = "Fitted regression line", values=c("a"="red", "b"="green"), labels=c("Age + age^2", "Age + age^2 + age^3")) + labs(x="Ego age", y="Mean probability of condom use", title = "Mean probability of condom use in main partnerships by respondent age \n with predicted condom use from a regression model ") + ylim(0, 1) + theme_title + plot_background
```

```{r condoms_pers_fit}
##PERSISTENT PARTNERS##
condoms_persXageinyrs <- sample_p12noprep_noalterprep %>% filter(!is.na(condoms_pers)) %>% group_by(age) %>% summarise(freq=n(), mean=mean(condoms_pers))

#Fit linear model to the mean condom use, adjusting for centered age and centered age squared
lm_condoms_persXage2 <- lm(condoms_pers ~ age_centered + age_cent_sq, data=sample_p12noprep_noalterprep)

#Fit linear model to the mean condom use, adjusting for centered age, centered age squared, and centered age cubed
lm_condoms_persXage3 <- lm(condoms_pers ~ age_centered + age_cent_sq + age_cent_cube, data=sample_p12noprep_noalterprep)

#Create a data frame with predictions from the models and age
condoms_pers_pred_age2 <- cbind.data.frame(prediction = predict(lm_condoms_persXage2, list(age_centered = sample_p12noprep_noalterprep$age_centered, age_cent_sq = (sample_p12noprep_noalterprep$age_centered)^2)), age = sample_p12noprep_noalterprep$age)

condoms_pers_pred_age3 <- cbind.data.frame(prediction = predict(lm_condoms_persXage3, list(age_centered = sample_p12noprep_noalterprep$age_centered, age_cent_sq = (sample_p12noprep_noalterprep$age_centered)^2, age_cent_cube = (sample_p12noprep_noalterprep$age_centered)^3)), age = sample_p12noprep_noalterprep$age)

#Plot
 ggplot() + geom_point(data=condoms_persXageinyrs, aes(x=age, y=mean, size=freq)) + geom_line(data=condoms_pers_pred_age2, aes(x=age, y=prediction, color="a")) + geom_line(data=condoms_pers_pred_age3, aes(x=age, y=prediction, color="b")) + scale_colour_manual(name = "Fitted regression line", values=c("a"="red", "b"="green"), labels=c("Age + age^2", "Age + age^2 + age^3")) + labs(x="Ego age", y="Mean probability of condom use", title = "Mean probability of condom use in persistent partnerships by respondent age \n with predicted condom use from a regression model ") + ylim(0, 1) + theme_title + plot_background
```

```{r condoms_inst_fit}
##INST PARTNERS##
condoms_instXageinyrs <- sample_p12noprep_noalterprep %>% filter(!is.na(condoms_inst)) %>% group_by(age) %>% summarise(freq=n(), mean=mean(condoms_inst))

#Fit linear model to the mean condom use, adjusting for centered age and centered age squared
lm_condoms_instXage2 <- glm(condoms_inst ~ age_centered + age_cent_sq, family=binomial(link="logit"), data=sample_p12noprep_noalterprep, subset = prep_use_r %in% "Never taken PrEP")

#Fit linear model to the mean condom use, adjusting for centered age, centered age squared, and centered age cubed
lm_condoms_instXage3 <- glm(condoms_inst ~ age_centered + age_cent_sq + age_cent_cube, family=binomial(link="logit"), data=sample_p12noprep_noalterprep, subset = prep_use_r %in% "Never taken PrEP")

#Create a data frame with predictions from the models and age
condoms_inst_pred_age2 <- cbind.data.frame(prediction = predict(lm_condoms_instXage2, list(age_centered = sample_p12noprep_noalterprep$age_centered, age_cent_sq = (sample_p12noprep_noalterprep$age_centered)^2)), age = sample_p12noprep_noalterprep$age)

condoms_inst_pred_age3 <- cbind.data.frame(prediction = predict(lm_condoms_instXage3, list(age_centered = sample_p12noprep_noalterprep$age_centered, age_cent_sq = (sample_p12noprep_noalterprep$age_centered)^2, age_cent_cube = (sample_p12noprep_noalterprep$age_centered)^3)), age = sample_p12noprep_noalterprep$age)

#Plot
 ggplot() + geom_point(data=condoms_instXageinyrs, aes(x=age, y=mean, size=freq)) + geom_line(data=condoms_inst_pred_age2, aes(x=age, y=prediction, color="a")) + geom_line(data=condoms_inst_pred_age3, aes(x=age, y=prediction, color="b")) + scale_colour_manual(name = "Fitted regression line", values=c("a"="red", "b"="green"), labels=c("Age + age^2", "Age + age^2 + age^3")) + labs(x="Ego age", y="Mean probability of condom use", title = "Mean probability of condom use in instantaneous partnerships by respondent age \n with predicted condom use from a regression model ") + ylim(0, 1) + theme_title + plot_background
```
