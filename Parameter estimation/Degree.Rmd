...
Because balancing the racial/ethnic mixing matrices resulted in adjustments to the mean degree for each racial/ethnic group, there may be inconsistencies in the model with the implied number of partnerships by region from the observed data. To evaluate this, we define region mixing parameters using both the observed data and estimates of degree defined as weighted averages of the racial/ethnic specific degree distributions.

First we will look at the full momentary degree distributions by region among HIV-negative men, using both observed and adjusted data
```{r degmatrix_KC}
...
num.mp.KC.adj <- deg.mp.KC.adj[1:2,1:3]*pop.totals[[3]]$total[1]
num.mp.OW.adj <- deg.mp.OW.adj[1:2,1:3]*pop.totals[[3]]$total[2]
num.mp.EW.adj <- deg.mp.EW.adj[1:2,1:3]*pop.totals[[3]]$total[3]
num.mp.tot <- deg.mp.adj[1:2,1:3]*sum(pop.totals[[3]]$total)
...
```{r prop.pers.region}
### Pers degree
pers.dist.region <- cbind.data.frame(region = c("Eastern WA", "King County", "Western WA"), proportion = c(sum(num.mp.EW[,2], 2*num.mp.EW[,3])/sum(num.mp.neg[,2], 2*num.mp.neg[,3]), sum(num.mp.KC[,2], 2*num.mp.KC[,3])/sum(num.mp.neg[,2], 2*num.mp.neg[,3]), sum(num.mp.OW[,2], 2*num.mp.OW[,3])/sum(num.mp.neg[,2], 2*num.mp.neg[,3])))

pers.dist.region.adj <- cbind.data.frame(region = c("Eastern WA", "King County", "Western WA"), proportion = c(sum(num.mp.EW.adj[,2], 2*num.mp.EW.adj[,3])/sum(num.mp.tot[,2], 2*num.mp.tot[,3]), sum(num.mp.KC.adj[,2], 2*num.mp.KC.adj[,3])/sum(num.mp.tot[,2], 2*num.mp.tot[,3]), sum(num.mp.OW.adj[,2], 2*num.mp.OW.adj[,3])/sum(num.mp.tot[,2], 2*num.mp.tot[,3])))


kable(pers.dist.region, col.names=c("Region", "Proportion"),  digits = 4, caption="Distribution of persistent degree by region - observed") %>% kable_styling(full_width=F, position="center") %>% column_spec(1:2, width = "4cm")

kable(pers.dist.region.adj, col.names=c("Region", "Proportion"),  digits = 4, caption="Distribution of persistent degree by region - adjusted") %>% kable_styling(full_width=F, position="center") %>% column_spec(1:2, width = "4cm")
