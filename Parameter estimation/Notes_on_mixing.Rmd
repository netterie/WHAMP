...
```
This file explores the implications of different strategies for specifying patterns of mixing and looks at the data from the internet survey in this context.
...
```{r racemixing}
    racemix_main <- table(sample$race_eth_m[sample$mrp_race_eth_m!="Dont know" & sample$mrp_type_ongoing %in% "Main"], sample$mrp_race_eth_m[sample$mrp_race_eth_m!="Dont know" & sample$mrp_type_ongoing %in% "Main"])
 racemix_pers <- table(sample$race_eth_m[sample$mrp_race_eth_m!="Dont know" & sample$mrp_type_ongoing %in% "Persistent"], sample$mrp_race_eth_m[sample$mrp_race_eth_m!="Dont know" & sample$mrp_type_ongoing %in% "Persistent"])
 racemix_inst <- table(sample$race_eth_m[sample$mrp_race_eth_m!="Dont know" & sample$mrp_type_r %in% "One time"], sample$mrp_race_eth_m[sample$mrp_race_eth_m!="Dont know" & sample$mrp_type_r %in% "One time"])
...
```{r agemixing}
    sample$age_cat_crude <- cut(sample$age, c(17, 24, 34, 44, 59), labels=c("18-24", "25-34", "35-44", "45-59"))
    sample$mrp_age_cat_crude <- cut(sample$mrp_ageinyears_approx, c(17, 24, 34, 44, 59), labels=c("18-24", "25-34", "35-44", "45-59"))    

    agemix_main <- table(sample$age_cat_crude[sample$mrp_type_ongoing %in% "Main"], sample$mrp_age_cat_crude[sample$mrp_type_ongoing %in% "Main"])
    agemix_pers <- table(sample$age_cat_crude[sample$mrp_type_ongoing %in% "Persistent"], sample$mrp_age_cat_crude[sample$mrp_type_ongoing %in% "Persistent"])
    agemix_inst <- table(sample$age_cat_crude[sample$mrp_type_r %in% "One time"], sample$mrp_age_cat_crude[sample$mrp_type_r %in% "One time"])
