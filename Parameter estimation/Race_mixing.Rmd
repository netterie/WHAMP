...
```{r}
#Conditional row probabilities
  racemix_pers <- prop.table(svytable(~hbo + mrp_hbo, sample_rake_all[sample_rake_all$variables$mrp_type_ongoing %in% "Persistent"], Ntotal=dim(sample[sample$mrp_type_ongoing %in% "Persistent",])[1], round=FALSE),1)
...
<span style=color:red>We decided to collapse the persistent and instantaneous networks in calculation of sufficient stats for race mixing. Figure out how to do this since I need to weight by mean degree. Would I calculate the conditional row probabilities using data from both networks, then weight by the mean degree for persistent partnesrhips in calculation of the target stats for that network? For isntantaneous partnerships, there isn't a mean degree, so do I just use the proportions reported with the MRP to obtain the target stats?</span>
