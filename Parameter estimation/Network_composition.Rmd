...
The age distribution of the network will be set to match the age distribution of all Washington males aged 18-59, based on the 2011-2015 American Community Survey 5-year Estimates [@WA-census]. This assumes that the age composition of MSM is similar to the age composition of all males. Of course, there may be differences due to migration patterns, but we lack reliable data to inform this. To weight data from MMP and other sources with data on HIV-positive men to the appropriate age distribution, we used data on the number of MSM living with HIV/AIDS in Washington by age to define the population totals. To weight data from the WHPP, we estimated the total number of MSM in each age group by applying the estimated percent of males who are MSM from @Grey2016 to ACS population data, and subtracted the number of MSM living with HIV/AIDS of each age group. This assumes that the proportion of males who are MSM is the same across age groups. 
Table \@ref(tab:agedist_pop) presents the resulting age distributions for all MSM, HIV-negative or unknown status MSM, and MSM living with diagnosed HIV/AIDS.
...
```{r agedist_pop}
    ...
    agedist.all <- wa_acs_agebyregion %>% select(-Frequency) %>% group_by(age_groups) %>% summarise(Freq = sum(numMSM)) %>% mutate(percent = Freq/sum(Freq))
    
    #HIV-positive only
    agedist.pos <- plwha_age %>% mutate(percent = Freq/sum(Freq))
    
    #HIV-negative only
    agedist.neg <- cbind.data.frame(age_groups = c("18-24", "25-29", "30-34", "35-39", "40-49", "50-59"), Freq = c(round(agedist.all$Freq - agedist.pos$Freq))) 
    agedist.neg$percent <- agedist.neg$Freq/sum(agedist.neg$Freq)

#Combine columns
    agedist <- cbind.data.frame("Age group" = agedist.neg$age_groups,  "All MSM" = agedist.all$percent, "MSM Diagnosed with HIV/AIDS" = agedist.pos$percent, "HIV-negative or unknown status MSM" = agedist.neg$percent)
...
As with age, we lack population-based data on the racial/ethnic distribution of the MSM population in Washington, so we assume the distribution of all males in Washington corresponds to the distribution of MSM. Because the regions in Washington are demographically diverse, we will representthe distirbution separately in each region. Table \@ref(tab:racebyregion) presents the racial/ethnic distribution of all MSM, approximated based on the distribution of all males aged 18-59 recorded in the 2011-2015 ACS [@WA-census], as well as the distribution separately by HIV status. The age distribution of diagnosed HIV-positive MSM is based on 2016 data on MSM living with HIV/AIDS in Washington. We calculated the age distribution of HIV-negative and unknown status MSM by applying the estimated percent of males who are MSM in each region [@Grey2016] to ACS population data for each region, and subtracted the number of MSM living with HIV/AIDS. This assumes that the proportion of males who are MSM is the same across racial/ethnic groups. Table \@ref(tab:racebyregion) presents the resulting racial/ethnic distributions for all MSM, HIV-negative or unknown status MSM, and MSM living with diagnosed HIV/AIDS.

```{r racebyregion}
#Define race/ethnic distribution by region for MSM
    #First get the regional distribution for MSM
...
#Race and region
    ...
    pop.raceregion.all <- pop.raceregion.all %>% group_by(region) %>% mutate(percent = Freq/sum(Freq))
    ...
    #HIV-negative only
    pop.raceregion.neg <- cbind.data.frame(region=c(rep("King County", 3), rep("Western WA", 3), rep("Eastern WA", 3)), hbo=c(rep(c("Hispanic", "Black", "Other"), 3)), Freq=c(round(pop.raceregion.all$Freq[pop.raceregion.all$region %in% "King County"] - pop.raceregion.pos$Freq[pop.raceregion.pos$region %in% "King County"]), round(pop.raceregion.all$Freq[pop.raceregion.all$region %in% "Western WA"] - pop.raceregion.pos$Freq[pop.raceregion.pos$region %in% "Western WA"]), round(pop.raceregion.all$Freq[pop.raceregion.all$region %in% "Eastern WA"] - pop.raceregion.pos$Freq[pop.raceregion.pos$region %in% "Eastern WA"])))
    pop.raceregion.neg <- pop.raceregion.neg %>% group_by(region) %>% mutate(percent = Freq/sum(Freq))
     #Specify order of factors
        pop.raceregion.neg$hbo <- factor(pop.raceregion.neg$hbo, levels = c("Hispanic", "Black", "Other"))
        pop.raceregion.neg$region <- factor(pop.raceregion.neg$region, levels = c("King County", "Western WA", "Eastern WA"))
        pop.raceregion.neg <- pop.raceregion.neg %>% arrange(region, hbo)

#Combine columns
    raceXregion <- cbind.data.frame("Region" =pop.raceregion.neg$region, "Race/ethnicity" = pop.raceregion.neg$hbo, "All MSM" = pop.raceregion.all$percent, "MSM Diagnosed with HIV/AIDS" = pop.raceregion.pos$percent, "HIV-negative or unknown status MSM" = pop.raceregion.neg$percent)
