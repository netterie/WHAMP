...
# Race
  ...
  ## Look at how many men who listed two or more races men identify as black 
    black.multi <- wa_acs_male18to59 %>% group_by(region) %>% filter(Age_group %in% "males.18to59") %>% summarise(total.black.alone = sum(Black_alone_nonHispanic), total.black.any = sum(Black_any_nonHispanic), total.multiple = sum(Multiple)) %>% mutate(total.black.multi = (total.black.any - total.black.alone), percent.blackmulti = total.black.multi / total.multiple)
...
# Race by region
  ## Define race groups - note: this is not perfect because the numbers for the individuals race/eth groups (Hispanic, NH white, NH black alone, NH Asian, NH Native Hawaiian/Other Pacific Islander, NH American Indian/Alaska Native, NH other, and NH two or more) do not sum to the total in each county. This is likely due to variability in the estimates for each group. Could keep the columns with the margin of error for the original data frame and work with those (details on how they are calculated here: https://www.census.gov/content/dam/Census/programs-surveys/acs/guidance/training-presentations/2016_MOE_Transcript_01.pdf. 
    ...
    wa_acs_male18to59_fullracedist <- wa_acs_male18to59 %>% mutate(blackmulti = Black_any_nonHispanic - Black_alone_nonHispanic) %>% mutate(nonblackmulti = Multiple - blackmulti) %>% select(county, region, Age_group, Hispanic, Black_any_nonHispanic, White, AIAN, Asian, NHOPI, Other, nonblackmulti, Total)
    wa_acs_fullracebyregion <- as.data.frame(wa_acs_male18to59_fullracedist %>% filter(Age_group %in% "males.18to59") %>% group_by(region) %>% summarise_at(vars(Hispanic:nonblackmulti), sum))
    wa_acs_fullracebyregion <- melt(wa_acs_fullracebyregion, id = "region")
    wa_acs_fullracebyregion <- wa_acs_fullracebyregion %>% group_by(region) %>% mutate(percent = value / sum(value))
    wa_acs_fullracebyregion_percent <- dcast(wa_acs_fullracebyregion, region ~ variable, value.var = "percent")
    ...
    wa_acs_male18to59_hbo <- wa_acs_male18to59 %>% select(county, region, Age_group, Black_any_nonHispanic, Hispanic, Total) %>% mutate(Other = Total - Black_any_nonHispanic - Hispanic)
    
    wa_acs_hbobyregion <- wa_acs_male18to59_hbo %>% filter(Age_group %in% "males.18to59") %>% group_by(region) %>% summarise_at(vars(Black_any_nonHispanic:Other), sum) %>% mutate(Percent_Hispanic = (Hispanic / Total), Percent_black = (Black_any_nonHispanic / Total), Percent_other = (Other / Total)) %>% select(region, Percent_Hispanic, Percent_black, Percent_other)
...
#-----------------------------------------------------------------------------------
    ...
    save(wa_acs_hbobyregion, file="Data/census_hboXregion.Rdata")
    save(wa_acs_fullracebyregion_percent, file="Data/census_fullraceXregion.Rdata")
