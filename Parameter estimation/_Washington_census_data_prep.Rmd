...
#Define race groups - note: this is not perfect because the numbers for the individuals race/eth groups (Hispanic, NH white, NH black alone, NH Asian, NH Native Hawaiian/Other Pacific Islander, NH American Indian/Alaska Native, NH other, and NH two or more) do not sum to the total in each county. This is likely due to variability in the estimates for each group. Could keep the columns with the margin of error for the original data frame and work with those (details on how they are calculated here: https://www.census.gov/content/dam/Census/programs-surveys/acs/guidance/training-presentations/2016_MOE_Transcript_01.pdf. But for now use use the point estimates and define other group as total - Hispanic - black alone or in combination
    ...
    #Ranges from 23% to 30%. Code as black
    
    # # Define race groups with the "other" category not collapsed  (for reasons specified above, this is some error in these totals)
    #      ## To run this code, need to load the first part of the "reweight_sample file to define the pop.region.all dataframe"
    # wa_acs_male18to59_fullracedist <- wa_acs_male18to59 %>% mutate(blackmulti = Black_any - Black_alone) %>% mutate(nonblackmulti = Multiple - blackmulti) %>% select(county, region, Age_group, Hispanic, Black_any, White, AIAN, Asian, NHOPI, Other, nonblackmulti, Total)
    # wa_acs_fullracebyregion <- wa_acs_male18to59_fullracedist %>% filter(Age_group %in% "males.18to59") %>% group_by(region) %>% summarise_at(vars(Hispanic:Total), sum)
    # wa_acs_fullracebyregion_percent <- wa_acs_fullracebyregion[,2:9] / wa_acs_fullracebyregion$Total
    # 
    # #Weighted by MSM population in each county
    # fullracebyregion <- rbind.data.frame(c(region = "Eastern WA", wa_acs_fullracebyregion_percent[1,]*pop.region.all$total[pop.region.all$region %in% "Eastern WA"]), c(region = "King County", wa_acs_fullracebyregion_percent[2,]*pop.region.all$total[pop.region.all$region %in% "King County"]), c(region = "Western WA", wa_acs_fullracebyregion_percent[3,]*pop.region.all$total[pop.region.all$region %in% "Western WA"]))
    # 
    # fullrace <- fullracebyregion %>% summarise_at(vars(Hispanic:nonblackmulti), sum)
    # fullrace_percent <- fullrace / sum(fullrace)
