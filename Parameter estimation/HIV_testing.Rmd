...
In previous MSM network models, never testers were defined as the proportion of men who never tested by age 40, since the models were capped at age 39. For this model, that may not be appropriate. In the plots below, we look at the proportion of men who report ever having tested by age. The first plot shows the proportion ever tested by single year, and the second plot groups age into 5-year bins. This analysis excludes men currently taking PrEP, as they will all have tested and their testing patterns will be represented differently in the model.

```{r evertest_byage}
  agetest <- sample %>% filter(!is.na(evertest_r)) %>% group_by(age) %>% summarise(prop = mean(evertest_r))
  ggplot(agetest, aes(x=age, y=prop)) + geom_point() + geom_smooth(method="loess") + labs(x="Age", y="Proprotion ever tested", title="Proportion of men ever testeed for HIV by age") + theme_title + plot_background
