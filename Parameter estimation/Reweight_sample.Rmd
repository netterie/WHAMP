...
#-----------------------------------------------------------------------------------
    ...
    sample_rake_neg <- rake(samplesvy, list(~hbo+region, ~age_groups), list(pop.raceregion.neg, pop.age.neg))
...
#-----------------------------------------------------------------------------------
    # sample_psraceregion <- postStratify(samplesvy, ~hbo + region, pop.raceregion.neg)
    # sample_rakeage <- rake(sample_psraceregion, list(~age_groups), list(pop.age.neg))
    sample_rake_all <- rake(samplesvy, list(~hbo+region, ~age_groups), list(pop.raceregion.all, pop.age.all))
    
     save(sample_rake_all, file="Data/InternetSurvey_reweighted_all.Rdata")

```

The weights resulting from raking to the joint distribution of race and region and the margins of age _among HIV-negative men_ are summarized below.
```{r ps_weights_neg}
    ...
    # svytable(~hbo + age_groups + region, sample_rake_neg[sample_rake_neg$prob %in% min(sample_rake_neg$prob)], round = TRUE)
    ...
    # svytable(~hbo + age_groups + region, sample_rake_neg[sample_rake_neg$prob %in% max(sample_rake_neg$prob)], round = TRUE)
...
The weights resulting from raking to the joint distribution of race and region and the margins of age _among all men_ are summarized below.
```{r ps_weights_all}
    ...
    #look at characteristics of those with highest weight
    # svytable(~hbo + age_groups + region, sample_rake_all[sample_rake_all$prob %in% min(sample_rake_all$prob)], round = TRUE)
    #     #look at number with these characteristics in original sample
    #     sample %>% filter(hbo %in% "Black" & region %in% "King County" & age_groups %in% "50-59") %>% summarise(Freq = n())
    # 
    # #look at characteristics of those with the lowest weight
    # svytable(~hbo + age_groups + region, sample_rake_all[sample_rake_all$prob %in% max(sample_rake_all$prob)], round = TRUE)
...
To check that this worked, the tables and graphs below show the distribution of the re-weighted HIV-negative sample by age, race/ethnicity, and region compared to the target population distributions.
...
#Make tables to compare margins and joint distributions
  #Age
    ...
    age_dist_reweighted <- as.vector(prop.table(svytable(~age_groups, sample_rake_neg, round=TRUE)))
