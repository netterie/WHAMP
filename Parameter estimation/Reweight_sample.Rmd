...
#Append probabilities to the main dataset, restricted to a version without NAs for race 
    sample$rake.prob <- sample_rake$prob
    sample$rake.wt <- 1/(sample_rake$prob)

#Reweight sample - comment out to avoid re-sampling each time the file is run
    # sample_reweighted <- sample %>% sample_n(10000, replace=TRUE, weight=rake.wt)
    # 
    # save(sample_reweighted, file="Data/InternetSurveySample_reweighted.Rdata")
...
```{r ps_weights}
    ...
    summary(sample_reweighted$rake.wt)
    
    # #look at characteristics of those with highest weight
    # table(sample_reweighted$hbo[sample_reweighted$rake.wt %in% (max(sample_reweighted$rake.wt))], (sample_reweighted$region[sample_reweighted$rake.wt %in% (max(sample_reweighted$rake.wt))]))
    ...
    # table(sample_reweighted$hbo[sample_reweighted$rake.wt %in% (min(sample_reweighted$rake.wt))], (sample_reweighted$region[sample_reweighted$rake.wt %in% (min(sample_reweighted$rake.wt))]))
    #     #look at number with these characteristics in original sample
...
#Make tables to compare margins and joint distributions
  #Age
    ...
    age_dist_reweighted <- as.vector(prop.table(table(sample_reweighted$age_groups)))
  ...
  #Margins of region
    ...
    region_dist_reweighted <- as.vector(prop.table(table(sample_reweighted$region)))
  ...
  #Margins of race
    ...
    race_dist_reweighted <- as.vector(prop.table(table(sample_reweighted$hbo)))
  ...
  #Race by region
    ...
    raceregion_dist_reweighted <- as.vector(prop.table(table(sample_reweighted$hbo, sample_reweighted$region), 2))
