...
We will define the rate of instantaneous partnerships by main/persistent degree, age, and HIV status. We also define stratified rates by race/ethnicity and region for use in calculating mixing (`nodematch`) target statistics and for `nodefactor` target statistics (which are needed for interpretation of the mixing terms).
...
#-----------------------------------------------------------------------------------
    ...
    neg.rateinst <- neg %>% select(pid, rate_inst, degree_main, degreecat_cas, region, hbo, age, hiv, mrp_hbo, mrp_type_ongoing, mrp_type, mrp_ageinyrs, mrp_hivpos, weight)
    ...
    pos.rateinst <- pos %>% select(pid, rate_inst, degree_main, degreecat_cas, region, hbo, age, hiv, mrp_hbo, mrp_type_ongoing, mrp_type, mrp_ageinyrs, mrp_hivpos, weight)

    tot.inst.unweighted <- rbind.data.frame(pos.rateinst, neg.rateinst)
    tot.inst <- svydesign(id=~1, data=tot.inst.unweighted, weights = ~weight) # Note this doesn't get the variance correct, but it gets the sample composition
    
    rm(pos_wts, pos, pos.rateinst, neg_wts, neg, neg.rateinst, artnet_pos_rake_w, sample_rake_neg_r)
...
tot.inst$variables$degree_main <- factor(tot.inst$variables$degree_main, levels = c(0, 1), labels = c("No main", "Main"))
    
# Re-order race and region variables
tot.inst$variables$hbo <- factor(tot.inst$variables$hbo, levels = c("Hispanic", "Black", "Other"), labels = c("Hispanic", "Black", "Other"))
tot.inst$variables$mrp_hbo <- factor(tot.inst$variables$mrp_hbo, levels = c("Hispanic", "Black", "Other"), labels = c("Hispanic", "Black", "Other"))
tot.inst$variables$region <- factor(tot.inst$variables$region, levels = c("King County", "Western WA", "Eastern WA"), labels = c("King County", "Western WA", "Eastern WA"))

# Define indicator of age 50+
tot.inst$variables$age50plus <- cut(tot.inst$variables$age, c(17, 49, 59), labels=c("18-49", "50-59")) 

```

Overall, the mean rate of instantaneous partnerships was `r round(svymean(~rate_inst, tot.inst, na.rm=TRUE)[[1]], 4)` per day. Among men under 50, the mean was `r round(svymean(~rate_inst, tot.inst[tot.inst$variables$age50plus %in% "18-49"], na.rm=TRUE)[[1]], 4)` per day, and among men 50 and older, the mean was `r round(svymean(~rate_inst, tot.inst[tot.inst$variables$age50plus %in% "50-59"], na.rm=TRUE)[[1]], 4)` per day.
...
```{r rate_instXdeg}
means <- svyby(~rate_inst, ~degree_main + degreecat_cas, tot.inst, svymean, na.rm=TRUE)
...
#    #Resample from the original survey dataframe 20 times using the raked weights
  ...
  # 
  #   for(i in 1:20){
  #       tot.inst_rwt <- tot.inst.unweighted %>% sample_n(1000000, replace=TRUE, weight=weight)
  # 
  #       #Age 18 to 49
  #       tot.inst_rwt_18to49 <- tot.inst_rwt %>% filter(age %in% c(18:49))
  #       tot.inst_rwt_18to49$rate_inst <- sort(tot.inst_rwt_18to49$rate_inst, na.last = TRUE)
  #       qsize.18to49=floor(sum(!is.na(tot.inst_rwt_18to49$rate_inst))/4)
  #       qrtmeans_18to49_i <- mean(tot.inst_rwt_18to49$rate_inst[1:qsize.18to49])
  #       qrtmeans_18to49_i[2] <- mean(tot.inst_rwt_18to49$rate_inst[(1*qsize.18to49+1):(2*qsize.18to49)])
  #       qrtmeans_18to49_i[3] <- mean(tot.inst_rwt_18to49$rate_inst[(2*qsize.18to49+1):(3*qsize.18to49)])
  #       qrtmeans_18to49_i[4] <- mean(tot.inst_rwt_18to49$rate_inst[(3*qsize.18to49+1):(sum(!is.na(tot.inst_rwt_18to49$rate_inst)))])
  # 
  #       qrtmeans_18to49[[i]] <- qrtmeans_18to49_i
  # 
  #       #Age 50-59
  #       tot.inst_rwt_50to59 <- tot.inst_rwt %>% filter(age %in% c(50:59))
  #       tot.inst_rwt_50to59$rate_inst <- sort(tot.inst_rwt_50to59$rate_inst, na.last = TRUE)
  #       qsize.50to59=floor(sum(!is.na(tot.inst_rwt_50to59$rate_inst))/4)
  #       qrtmeans_50to59_i <- mean(tot.inst_rwt_50to59$rate_inst[1:qsize.50to59])
  #       qrtmeans_50to59_i[2] <- mean(tot.inst_rwt_50to59$rate_inst[(1*qsize.50to59+1):(2*qsize.50to59)])
  #       qrtmeans_50to59_i[3] <- mean(tot.inst_rwt_50to59$rate_inst[(2*qsize.50to59+1):(3*qsize.50to59)])
  #       qrtmeans_50to59_i[4] <- mean(tot.inst_rwt_50to59$rate_inst[(3*qsize.50to59+1):(sum(!is.na(tot.inst_rwt_50to59$rate_inst)))])
  # 
...
```
## Instantaneous partnerships by HIV status
```{r rate_instXhbo}
means.hiv <- svyby(~rate_inst, ~hiv, tot.inst, svymean, na.rm=TRUE)
meansXhiv <- cbind.data.frame("status" = means.hiv$hiv, "Mean" = round(means.hiv$rate_inst, 6))

#Reorder race var to alphabetical order
meansXhiv$status <- factor(meansXhiv$status, levels = c("0", "1"), labels = c("HIV-negative", "HIV-positive"))

#Make kable
kable(meansXhiv, caption="Mean rate of instantaneous partnerships per day by HIV status", digits=c(6)) %>% kable_styling(full_width=F, position="center") %>% column_spec(1:2, width = "4cm")

#save means
save(meansXhiv, file="Data/rate_inst_hiv.Rdata")
```

## Instantaneous partnerships by race/ethnicity
```{r rate_instXhbo}
means.hbo <- svyby(~rate_inst, ~hbo, tot.inst, svymean, na.rm=TRUE)
...
```{r rate_instXregion}
means.region <- svyby(~rate_inst, ~region, tot.inst, svymean, na.rm=TRUE)
...
```{r}
num.inst.EW <- meansXregion$Mean[1]*svytable(~region, tot.inst, round=FALSE)[3]
num.inst.KC <- meansXregion$Mean[2]*svytable(~region, tot.inst, round=FALSE)[1]
num.inst.OW <- meansXregion$Mean[3]*svytable(~region, tot.inst, round=FALSE)[2]
num.inst.neg <- svymean(~rate_inst, tot.inst[tot.inst$variables$hiv %in% 0], na.rm=TRUE)[[1]]*svytable(~hiv, tot.inst, round=FALSE)[1]
